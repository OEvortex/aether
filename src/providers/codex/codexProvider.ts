    ...
    implements LanguageModelChatProvider
{
    ...
    static createAndActivate(context: vscode.ExtensionContext): {
        ...
        setTimeout(() => {
            CodexAuth.isLoggedIn().then((isLoggedIn) => {
                if (isLoggedIn) {
                    Logger.info(
                        '[Codex] User is logged in, firing model change event'
                    );
    ...
    async provideLanguageModelChatInformation(
        ...
        try {
            ...
            const modelInfos: LanguageModelChatInformation[] =
                this.cachedModels.map((model) => {
                    return {
                        id: model.id,
                        name: model.name,
                        vendor: 'chp.codex',
    ...
    async provideLanguageModelChatResponse(
        ...
        try {
            ...
            const accounts =
                this.accountManager.getAccountsByProvider(providerKey);
            const activeAccount =
                this.accountManager.getActiveAccount(providerKey);
            ...
            for (const account of accountCandidates) {
                ...
                if (!authContext.accessToken) {
                    lastError = new Error(
                        'Not logged in to Codex. Please login first.'
                    );
                    continue;
                }
                ...
                try {
                    Logger.info(
                        `Codex Provider processing request: ${model.name}`
                    );
                    Logger.info(
    ...
    private async resolveAuthContext(account?: Account): Promise<{
        ...
        return {
            ...
            projectId:
                metadataProjectId || (await CodexAuth.getProjectId()) || '',
