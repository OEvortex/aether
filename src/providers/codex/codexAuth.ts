        ...
        const req = https.request(options, (res) => {
            ...
            res.on('end', () => {
                try {
                    ...
                    reject(
                        new Error(`Failed to parse token response: ${body}`)
                    );
                }
            });
        });
    ...
    return new Promise((resolve, reject) => {
        ...
        const server = http.createServer((req, res) => {
            if (req.url?.startsWith('/auth/callback')) {
                const url = new URL(
                    req.url,
                    `http://localhost:${CODEX_CALLBACK_PORT}`
                );
    ...
    try {
        ...
        if (tokenResp.id_token) {
            ...
            if (claims) {
                ...
                Logger.info(
                    `  - projectId (direct): ${projectId || 'NOT_FOUND'}`
                );
                Logger.info(
        ...
        if (action.action === 'refresh') {
            ...
            if (stored) {
                try {
                    ...
                    const newToken = await CodexAuth.refreshToken(
...
async function doCodexLoginAndSave(isAddingNewAccount: boolean): Promise<void> {
    ...
    if (result) {
        ...
        Logger.info(
            `  - organization_id: ${authData.organization_id || 'EMPTY'}`
        );
...
export class CodexAuth {
                ...
                res.on('end', async () => {
                    try {
                        ...
                        if (
                            ...
                            const stored =
                                await ApiKeyManager.getApiKey('codex');
                            const expiresAt = new Date(
    ...
    static async getAccountId(): Promise<string> {
        ...
        try {
            ...
            Logger.error(
                '[codex] getAccountId: Failed to parse auth data',
                err
            );
            return '';
        }
    ...
    static async getOrganizationId(): Promise<string> {
        ...
        try {
            ...
            Logger.error(
                '[codex] getOrganizationId: Failed to parse auth data',
                err
            );
            return '';
        }
    ...
    static async getProjectId(): Promise<string> {
        ...
        try {
            ...
            Logger.error(
                '[codex] getProjectId: Failed to parse auth data',
                err
            );
            return '';
        }
