/*---------------------------------------------------------------------------------------------
 *  MimoCode Provider
 *  Free AI provider via Xiaomi Mimo free API - bootstraps JWT automatically
 *--------------------------------------------------------------------------------------------*/
...
{
    private readonly handler = new MimoCodeHandler();
    ...
    override async provideLanguageModelChatInformation(
        ...
        try {
            ...
            Logger.warn(
                `[MimoCode] Failed to fetch models: ${error instanceof Error ? error.message : String(error)}`
            );
            const staticModels = [...(this.providerConfig.models || [])];
            this.cachedModelConfigs = staticModels;
            return staticModels.map((m) => this.modelConfigToInfo(m));
    ...
    ): { provider: MimoCodeProvider; disposables: vscode.Disposable[] } {
        Logger.trace(`${providerConfig.displayName} provider activated!`);

        const provider = new MimoCodeProvider(
            context,
            providerKey,
            providerConfig
        );
