...
function getKnoxMaxCompletionTokens(model: FetchedModel): number | undefined {
    const top = model.top_provider as
...
export class DynamicModelProvider extends GenericModelProvider {
    ...
    private async refreshModelsAsync(silent: boolean): Promise<void> {
        ...
        try {
            ...
            if (!apiKey && this.knownConfig.openModelEndpoint) {
                ...
                Logger.trace(
                    `[${this.providerKey}] API key required for model fetch`
                );
                return;
            }
            ...
            if (models.length > 0) {
                ...
                Logger.warn(
                    `[${this.providerKey}] No models returned from API`
                );
            }
        } catch (err) {
            Logger.error(
    ...
    private async updateModels(models: FetchedModel[]): Promise<void> {
        try {
            ...
            const oldModelsJson = JSON.stringify(
                this.baseProviderConfig.models
            );
            ...
            if (oldModelsJson !== newModelsJson) {
                ...
                const modelsWithAutoDetection = overriddenConfig.models.map(
                    (model) => {
                        if (
                            requiresThinkingContentRoundTrip(
