...
export class OpenCodeGoProvider
    ...
    override async provideLanguageModelChatInformation(
        ...
        if (
            ...
            return this.cachedModelConfigs.map((m) => this.modelConfigToInfo(m));
        }
    ...
    override async provideLanguageModelChatResponse(
        ...
        await rateLimiter.executeWithRetry(async () => {
            await this.executeRequest(model, messages, options, progress, token);
    ...
    private async executeRequest(
        ...
        const modelConfig = this.cachedModelConfigs.find(
            (m) => m.id === model.id
