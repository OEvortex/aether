...
const GEMINI_REASONING_CONTROL_FIELDS = new Set([
    'reasoning',
    'reasoningEffort',
    'reasoning_effort'
]);

const GEMINI_DASHSCOPE_HEADERS = {
    'X-Gemini-AuthType': 'gemini-oauth',
    'X-Gemini-UserAgent': getUserAgent()
};
    ...
    extends GenericModelProvider
{
    private buildGeminiModelConfig(
        ...
        const outputCap = GEMINI_MODEL_OUTPUT_LIMITS[normalizedModelId];
        const sanitizedExtraBody = modelConfig.extraBody
            ? Object.fromEntries(
                  Object.entries(modelConfig.extraBody).filter(
                      ([key]) => !GEMINI_REASONING_CONTROL_FIELDS.has(key)
                  )
              )
            : undefined;
        ...
        const loginCommand = vscode.commands.registerCommand(
            ...
            async () => {
                ...
                try {
                    ...
                    Logger.debug(
                        '[geminicli] No existing credentials, starting OAuth flow'
                    );
                }
                ...
                if (part instanceof vscode.LanguageModelTextPart) {
                    ...
                    if (thinking && !hideThinkingInUI) {
                        if (!currentThinkingId) {
                            currentThinkingId = `gemini_thinking_${Date.now()}`;
                        }
                        progress.report(
                    ...
                    if (openStart !== -1 && closeEnd === -1) {
                        ...
                        if (beforeOpen && beforeOpen.length > 0) {
                            ...
                            progress.report(
                                new vscode.LanguageModelTextPart(beforeOpen)
                            );
                        }
                        ...
                        if (trailing && trailing.length > 0) {
                            ...
                            progress.report(
                                new vscode.LanguageModelTextPart(trailing)
                            );
                        }
                    }
                } else {
        ...
        try {
            ...
            const tryAccountRequest = async (
                ...
                if (!accountAccessToken) {
                    const creds = (await accountManager.getCredentials(
                        account.id
                    ...
                    if ('accessToken' in creds) {
                        accountAccessToken = (creds as OAuthCredentials)
                            .accessToken;
                    } else if ('apiKey' in creds) {
                        accountAccessToken = (creds as ApiKeyCredentials)
                            .apiKey;
                    }
                    if (!accountAccessToken) {
            ...
            if (accounts && accounts.length > 0) {
                ...
                const activeAccount =
                    accountManager.getActiveAccount('geminicli');
                ...
                for (const account of accountsToTry) {
                    ...
                    if (
                        loadBalanceEnabled &&
                        this.isInsufficientQuotaError(result.error)
                    ) {
                        switchedAccount = true;
                        continue;
                    }
            ...
            try {
                ...
                Logger.trace(`[geminicli] Thinking flush failed: ${String(e)}`);
            }
        }
    }
}
