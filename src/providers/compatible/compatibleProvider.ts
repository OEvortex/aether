...
export class CompatibleProvider extends GenericModelProvider {
        ...
        try {
            ...
            const modelConfigs: ModelConfig[] = models.map((model) => {
                ...
                return {
                    ...
                    tooltip:
                        model.tooltip || `${model.name} (${model.sdkMode})`,
            ...
            Logger.error(
                'Failed to get Compatible Provider configuration:',
                error
            );
        ...
        try {
            ...
            let modelInfos = currentConfig.models.map((model) => {
                ...
                if (model.provider) {
                    ...
                    const provider =
                        configProviders[
                            model.provider as keyof typeof configProviders
                        ];
    ...
    protected override updateModelCacheAsync(
        apiKeyHash: string,
        _models?: LanguageModelChatInformation[]
    ): void {
        (async () => {
            try {
                const currentConfig = this.providerConfig;
    ...
    async provideLanguageModelChatResponse(
        ...
        if (rememberLastModel) {
            this.modelInfoCache
                ...
                .catch((err) =>
                    Logger.warn(
                        '[compatible] Failed to save model selection:',
                        err
                    )
                );
        }
        ...
        try {
            ...
            const modelConfig = currentConfig.models.find(
                (m) => m.id === model.id
            );
            if (!modelConfig) {
            ...
            const sdkName =
                sdkMode === 'anthropic' ? 'Anthropic SDK' : 'OpenAI SDK';
            ...
            try {
                ...
                Logger.info(
                    `Compatible Provider: ${model.name} Request completed`
                );
            }
        } catch (error) {
            Logger.error(
                'Compatible Provider failed to process request:',
                error
            );
            throw error;
        }
            ...
            vscode.commands.registerCommand(
                ...
                async () => {
                    try {
                        ...
                        Logger.error(
                            'Failed to manage Compatible models:',
                            error
                        );
