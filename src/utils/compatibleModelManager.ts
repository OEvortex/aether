...
function isBackButtonClick(value: unknown): value is BackButtonClick {
    return (
        typeof value === 'object' && (value as BackButtonClick)?.back === true
    );
}
...
export class CompatibleModelManager {
    ...
    private static normalizeLegacyTokenLimits(
        ...
        if (model.maxInputTokens === 128000 && model.maxOutputTokens === 4096) {
            return {
                ...model,
    ...
    static async updateModel(
        ...
        const index = CompatibleModelManager.models.findIndex(
            (m) => m.id === id
        );
        if (index === -1) {
        ...
        if (!CompatibleModelManager.models[index]) {
            throw new Error(
                `Model data corrupted, cannot update model ID "${id}"`
            );
        }
    ...
    static async removeModel(id: string): Promise<void> {
        ...
        if (!removedModel) {
            throw new Error(
                `Model data corrupted, cannot delete model ID "${id}"`
            );
        }
    ...
    static async removeModels(
        ...
        for (const id of validIds) {
            ...
            if (index !== -1) {
                ...
                CompatibleModelManager.models.splice(index, 1);
                Logger.info(
                    `Deleted custom model: ${removedModel?.name || id}`
                );
                success++;
            } else {
                failed++;
            ...
            if (builtinProviders.length > 0) {
                ...
                providerChoices.push(
                    ...builtinProviders.map((provider) => ({
                        ...
                        description:
                            configProviders[
                                provider as keyof typeof configProviders
                            ]?.displayName
                    }))
                );
            }
    ...
    private static async configureModels(): Promise<void> {
        while (true) {
            ...
            const quickPick =
                vscode.window.createQuickPick<ModelQuickPickItem>();
    ...
    private static async showBulkDeletePicker(): Promise<void> {
        ...
        const items: DeleteModelQuickPickItem[] =
            CompatibleModelManager.models.map((model) => ({
                label: model.name,
                description: model.id,
        ...
        const quickPick =
            vscode.window.createQuickPick<DeleteModelQuickPickItem>();
