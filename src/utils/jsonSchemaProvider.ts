...
const SCHEMA_URI = 'aether-settings://root/schema.json';
let schemaProvider: vscode.Disposable | null = null;
...
function getAllAvailableProviders(): {
    ...
    try {
        ...
        for (const model of customModels) {
            if (
                model.provider?.trim() &&
                !providerIds.includes(model.provider)
            ) {
                ...
                items: {
                    ...
                    properties: {
                        ...
                        extraBody: {
                            ...
                            additionalProperties: {
                                description:
                                    'Extra request body parameter value'
                            }
                        }
                    },
                    required: ['id'],
 ...
 */
export function getProviderOverridesSchema(): JSONSchema7 {
    ...
    for (const [providerKey, config] of Object.entries(providerConfigs)) {
        patternProperties[`^${providerKey}$`] = createProviderSchema(
            providerKey,
            config
        );
    }
 ...
 */
function generateSchemaHash(schema: JSONSchema7): string {
...
function updateSchema(): void {
    try {
        ...
        const uri = vscode.Uri.parse(SCHEMA_URI);
        vscode.workspace.textDocuments.forEach((doc) => {
            if (doc.uri.toString() === SCHEMA_URI) {
...
export function initialize(): void {
    ...
    schemaProvider = vscode.workspace.registerTextDocumentContentProvider(
        'aether-settings',
        {
            provideTextDocumentContent: (uri: vscode.Uri): string => {
                if (uri.toString() === SCHEMA_URI) {
                    const schema = getProviderOverridesSchema();
...
/**
 * JSON Schema Provider (deprecated class-like interface for backward compatibility)
 */
export const JsonSchemaProvider = {
    initialize,
    getProviderOverridesSchema,
    dispose
};
