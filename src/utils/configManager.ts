...
export class ConfigManager {
    ...
    static getConfig(): CHPConfig {
        ...
        ConfigManager.cache = {
            ...
            hideThinkingInUI:
                ConfigManager.getHideThinkingInUIFromConfig(config),
            zhipu: {
                search: {
                    enableMCP: config.get<boolean>(
    ...
    private static validateTopP(value: number): number {
        if (Number.isNaN(value) || value < 0 || value > 1) {
            Logger.warn(
                `Invalid topP value: ${value}, using default value 1.0`
            );
            return 1.0;
        }
        return value;
    ...
    private static buildProviderOverrides(
        ...
        for (const providerKey of mergedProviderKeys) {
            const sdkMode = config
                .get<string>(`${providerKey}.sdkMode`, '')
                .trim();
        ...
        for (const [key, override] of Object.entries(configuredOverrides)) {
            ...
            const { baseUrl: _ignoredBaseUrl, ...supportedOverride } =
                override as ProviderOverride & { baseUrl?: string };
            ...
            for (const modelOverride of override.models) {
                ...
                if (existingModelIndex >= 0) {
                    ...
                    if (modelOverride.outputThinking !== undefined) {
                        existingModel.outputThinking =
                            modelOverride.outputThinking;
                        Logger.debug(
                    ...
                    const newModel: ModelConfig = {
                        ...
                        tooltip:
                            fullConfig?.tooltip ||
                            `User custom model: ${modelOverride.id}`,
                        maxInputTokens: modelOverride.maxInputTokens || 128000,
                        maxOutputTokens: modelOverride.maxOutputTokens || 8192,
                        capabilities: {
                            toolCalling:
                                modelOverride.capabilities?.toolCalling ??
                                false,
                            imageInput:
                                modelOverride.capabilities?.imageInput ?? false
                        },
                        ...(modelOverride.model && {
                            model: modelOverride.model
                        }),
                        ...(modelOverride.sdkMode && {
                            sdkMode: modelOverride.sdkMode
                        }),
