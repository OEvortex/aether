...
import type { ExtensionInstallMetadata, MCPServerConfig } from '../index.js';
import {
...
describe('extension tests', () => {
    ...
    describe('loadExtension', () => {
        ...
        it('should use default AGENTS.md when contextFileName is empty array', async () => {
            ...
            const ext = extensions.find(
                (e) => e.config.name === 'ext-empty-context'
            );
        ...
        it('should filter trust out of mcp servers', async () => {
            ...
            expect(
                extensions[0].mcpServers?.['test-server']?.trust
            ).toBeUndefined();
            // config.mcpServers should still have trust (original config)
            expect(
                extensions[0].config.mcpServers?.['test-server']?.trust
            ).toBe(true);
        });
    });
    ...
    describe('enableExtension / disableExtension', () => {
        it('should disable an extension at the user scope', async () => {
            ...
            expect(manager.isEnabled('my-extension', tempWorkspaceDir)).toBe(
                false
            );
        });
    ...
    describe('validateExtensionOverrides', () => {
        ...
        it('should mark only the enabled extensions as active', async () => {
            ...
            expect(extensions.find((e) => e.name === 'ext1')?.isActive).toBe(
                true
            );
            expect(extensions.find((e) => e.name === 'ext2')?.isActive).toBe(
                false
            );
            expect(extensions.find((e) => e.name === 'ext3')?.isActive).toBe(
                true
            );
        });
    ...
    describe('extensionManager utility functions', () => {
        describe('validateName', () => {
            ...
            it('should reject names with invalid characters', () => {
                ...
                expect(() => validateName('my@ext')).toThrow(
                    'Invalid extension name'
                );
            });
                    ...
                    PreToolUse: [
                        {
                            ...
                            hooks: [
                                { type: 'command', command: 'echo "hooks dir"' }
                            ]
                        }
                    ]
                })
