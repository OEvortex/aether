...
export class ExtensionManager {
    ...
    constructor(options: ExtensionManagerOptions) {
        ...
        this.enabledExtensionNamesOverride =
            options.enabledExtensionOverrides?.map((name) =>
                name.toLowerCase()
            ) ?? [];
    ...
    validateExtensionOverrides(extensions: Extension[]): void {
        for (const name of this.enabledExtensionNamesOverride) {
            ...
            if (
                !extensions.some(
                    (ext) =>
                        ext.config.name.toLowerCase() === name.toLowerCase()
                )
            ) {
    ...
    isEnabled(extensionName: string, currentPath?: string): boolean {
        ...
        for (const rule of allOverrides) {
            ...
            if (
                override.matchesPath(ensureLeadingAndTrailingSlash(checkPath))
            ) {
            ...
            try {
                ...
                if (isUpdate && installMetadata.autoUpdate) {
                    ...
                    const settingsAreEqual =
                        ...
                        [...oldSettings].every((value) =>
                            newSettings.has(value)
                        );
        ...
        for (const extension of extensions) {
            try {
                const installMetadata: ExtensionInstallMetadata = {
                    ...
                    type: 'local',
                    originSource:
                        extension.installMetadata?.originSource || 'Aether'
                };
