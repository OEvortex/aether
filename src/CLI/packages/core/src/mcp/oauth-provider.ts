...
export class MCPOAuthProvider {
    ...
    constructor(
        tokenStorage: MCPOAuthTokenStorage = new MCPOAuthTokenStorage()
    ) {
...
✓ No authorization URL; using OAuth discovery`);
            ...
            try {
                const headers: HeadersInit = OAuthUtils.isSSEEndpoint(
                    mcpServerUrl
                )
                    ...
                    if (wwwAuthenticate) {
                        ...
                        if (discoveredConfig) {
                            ...
                            config = {
                                ...config,
                                authorizationUrl:
                                    discoveredConfig.authorizationUrl,
        ...
        try {
            ...
            debugLogger.error(
                `Failed to save token: ${getErrorMessage(saveError)}`
            );
            throw saveError;
        }
    ...
    async getValidToken(
        ...
        if (!this.tokenStorage.isTokenExpired(token)) {
            debugLogger.debug(
                `Returning valid token for server: ${serverName}`
            );
        ...
        if (token.refreshToken && config.clientId && credentials.tokenUrl) {
            try {
                ...
                debugLogger.error(
                    `Failed to refresh token: ${getErrorMessage(error)}`
                );
