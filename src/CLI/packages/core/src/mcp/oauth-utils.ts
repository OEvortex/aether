...
export class OAuthUtils {
    ...
    static async discoverAuthorizationServerMetadata(
        ...
        for (const endpoint of endpointsToTry) {
            const authServerMetadata =
                await OAuthUtils.fetchAuthorizationServerMetadata(endpoint);
    ...
    static async discoverOAuthConfig(
        ...
        try {
            ...
            const wellKnownUrls = OAuthUtils.buildWellKnownUrls(
                serverUrl,
                false
            );
            ...
            let resourceMetadata =
                await OAuthUtils.fetchProtectedResourceMetadata(
            ...
            if (!resourceMetadata) {
                ...
                if (url.pathname && url.pathname !== '/') {
                    const pathBasedUrls = OAuthUtils.buildWellKnownUrls(
                        serverUrl,
                        true
                    );
                    resourceMetadata =
            ...
            debugLogger.debug(
                `Trying OAuth discovery fallback at ${serverUrl}`
            );
            const authServerMetadata =
