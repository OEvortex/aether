...
describe('OAuthUtils', () => {
    ...
    describe('isSSEEndpoint', () => {
        it('should return true for SSE endpoints', () => {
            expect(OAuthUtils.isSSEEndpoint('https://example.com/sse')).toBe(
                true
            );
            expect(
                OAuthUtils.isSSEEndpoint('https://example.com/api/v1/sse')
            ).toBe(true);
        });

        it('should return true for non-MCP endpoints', () => {
            expect(OAuthUtils.isSSEEndpoint('https://example.com/api')).toBe(
                true
            );
        });

        it('should return false for MCP endpoints', () => {
            expect(OAuthUtils.isSSEEndpoint('https://example.com/mcp')).toBe(
                false
            );
            expect(
                OAuthUtils.isSSEEndpoint('https://example.com/api/mcp/v1')
            ).toBe(false);
        });
    });
    ...
    describe('buildResourceParameter', () => {
        ...
        it('should handle root URL consistently', () => {
            ...
            expect(
                OAuthUtils.buildResourceParameter('https://example.com')
        ...
        it('should use scopes from protected resource metadata when available', async () => {
            ...
            const mockAuthServerMetadata: OAuthAuthorizationServerMetadata = {
                ...
                authorization_endpoint:
                    'https://www.modelscope.cn/oauth/authorize',
