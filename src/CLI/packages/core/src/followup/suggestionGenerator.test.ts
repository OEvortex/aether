...
describe('shouldFilterSuggestion', () => {
    ...
    it('filters multiple sentences', () => {
        expect(shouldFilterSuggestion('Run the tests. Then commit.')).toBe(
            true
        );
    });
    ...
    it('does not false-positive on evaluative substrings', () => {
        expect(shouldFilterSuggestion('run nicely formatted tests')).toBe(
            false
        );
        expect(shouldFilterSuggestion('fix the greatest issue')).toBe(false);
        expect(shouldFilterSuggestion('create thanksgiving banner')).toBe(
            false
        );
    });
