...
export class CoreToolScheduler {
                    ...
                    if (currentCall.status === 'awaiting_approval') {
                        ...
                        if (waitingCall.confirmationDetails.type === 'edit') {
                            resultDisplay = {
                                ...
                                fileName:
                                    waitingCall.confirmationDetails.fileName,
                                originalContent:
                                    waitingCall.confirmationDetails
                                        .originalContent,
                                newContent:
                                    waitingCall.confirmationDetails.newContent
                            };
                        } else if (
                            waitingCall.confirmationDetails.type === 'plan'
                        ) {
                            resultDisplay = {
                                type: 'plan_summary',
                                message:
            ...
            return new Promise((resolve, reject) => {
                const abortHandler = () => {
                    ...
                    if (index > -1) {
                        ...
                        reject(
                            new Error('Tool call cancelled while in queue.')
                        );
                    }
                };
        ...
        try {
            ...
            for (const reqInfo of requestsToProcess) {
                ...
                if (!toolInstance) {
                    ...
                    const errorMessage = this.getToolNotFoundMessage(
                        reqInfo.name
                    );
                    newToolCalls.push({
                        status: 'error',
            ...
            for (const toolCall of newToolCalls) {
                ...
                try {
                    ...
                    const pm = this.config.getPermissionManager?.();
                    const toolParams = invocation.params as Record<
                        string,
                        unknown
                    >;
                    ...
                    if (
                        ...
                        if (
                            ...
                            confirmationDetails.ideConfirmation.then(
                                (resolution) => {
    ...
    async handleConfirmationResponse(
        ...
        const toolCall = this.toolCalls.find(
            (c) =>
                c.request.callId === callId && c.status === 'awaiting_approval'
        );
        ...
        if (outcome === ToolConfirmationOutcome.Cancel || signal.aborted) {
            ...
            if (isModifiableDeclarativeTool(waitingToolCall.tool)) {
                ...
                >(
                    ...
                    modifyContext as ModifyContext<
                        typeof waitingToolCall.request.args
                    >,
                    editorType,
                    signal,
