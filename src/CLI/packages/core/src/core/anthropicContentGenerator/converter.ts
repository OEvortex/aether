...
export class AnthropicContentConverter {
    ...
    convertAnthropicResponseToGemini(
        ...
        for (const block of response.content || []) {
            ...
            if (blockType === 'text') {
                ...
                const thinking =
                    typeof (block as { thinking?: string }).thinking ===
                    'string'
                        ? (block as { thinking?: string }).thinking
                        : '';
                const signature =
                    typeof (block as { signature?: string }).signature ===
                    'string'
        ...
        for (const part of parts) {
            ...
            if ('text' in part && 'thought' in part && part.thought) {
                if (role === 'assistant') {
                    ...
                    contentBlocks.push(
                        thinkingBlock as AnthropicContentBlockParam
                    );
                }
            }
            ...
            if ('functionCall' in part && part.functionCall) {
                if (role === 'assistant') {
                    contentBlocks.push({
                        ...
                        input:
                            (part.functionCall.args as Record<
                                string,
                                unknown
                            >) || {}
                    });
    ...
    private createMediaBlockFromPart(
        ...
        if (part.inlineData?.mimeType && part.inlineData?.data) {
            if (
                this.isSupportedAnthropicImageMimeType(part.inlineData.mimeType)
            ) {
                return {
                    type: 'image',
                    source: {
        ...
        if (part.fileData?.mimeType && part.fileData?.fileUri) {
            ...
            if (
                this.isSupportedAnthropicImageMimeType(part.fileData.mimeType)
            ) {
                return {
                    type: 'image',
                    source: {
