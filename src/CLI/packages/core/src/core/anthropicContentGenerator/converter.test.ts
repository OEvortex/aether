...
describe('AnthropicContentConverter', () => {
    ...
    describe('convertGeminiToolsToAnthropic', () => {
        ...
        it('resolves CallableTool.tool() and converts its functionDeclarations', async () => {
            ...
            const result =
                await converter.convertGeminiToolsToAnthropic(callable);
    ...
    describe('mapAnthropicFinishReasonToGemini', () => {
        it('maps known reasons', () => {
            ...
            expect(
                converter.mapAnthropicFinishReasonToGemini('max_tokens')
            ).toBe(FinishReason.MAX_TOKENS);
            expect(
                converter.mapAnthropicFinishReasonToGemini('content_filter')
            ).toBe(FinishReason.SAFETY);
        });

        it('returns undefined for null/empty', () => {
            expect(
                converter.mapAnthropicFinishReasonToGemini(null)
            ).toBeUndefined();
            expect(
                converter.mapAnthropicFinishReasonToGemini('')
            ).toBeUndefined();
        });
    });
    ...
    describe('enableCacheControl', () => {
        ...
        it('does not add cache_control to messages when disabled', () => {
            ...
            const { messages } =
                noCacheConverter.convertGeminiRequestToAnthropic({
                    model: 'models/test',
                    contents: 'Hello'
                });
