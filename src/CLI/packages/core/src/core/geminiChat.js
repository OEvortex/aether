...
function extractCuratedHistory(comprehensiveHistory) {
    if (comprehensiveHistory === undefined ||
                ...
                for (let attempt = 0; attempt < INVALID_CONTENT_RETRY_OPTIONS.maxAttempts; attempt++) {
                    try {
                        ...
                        if (isRateLimit &&
                            ...
                            const message = parseAndFormatApiError(error instanceof Error
                                ? error.message
                                : String(error));
                        ...
                        if (isTransientStreamError &&
                            ...
                            attempt--;
                            await delay(delayMs, params.config?.abortSignal)
                                .promise;
                            continue;
                        }
                        ...
                        if (isContentError) {
                            if (attempt <
                                INVALID_CONTENT_RETRY_OPTIONS.maxAttempts - 1) {
                                logContentRetry(self.config, new ContentRetryEvent(attempt, error.type, INVALID_CONTENT_RETRY_OPTIONS.initialDelayMs, model));
                                await delay(INVALID_CONTENT_RETRY_OPTIONS.initialDelayMs *
                                    (attempt + 1), params.config?.abortSignal).promise;
                                continue;
                            }
                        }
                        break;
    ...
    async *processStreamResponse(model, streamResponse) {
        ...
        for await (const chunk of streamResponse) {
            ...
            if (chunk.usageMetadata) {
                ...
                const lastPromptTokenCount = usageMetadata.totalTokenCount ||
