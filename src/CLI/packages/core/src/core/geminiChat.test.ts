...
describe('GeminiChat', async () => {
    ...
    describe('sendMessageStream', () => {
        it('should succeed if a tool call is followed by an empty part', async () => {
            ...
            ).mockResolvedValue(streamWithToolCall);
        ...
        it('should fail if the stream ends with an empty part and has no finishReason', async () => {
            ...
            try {
                ...
                vi.mocked(
                    mockContentGenerator.generateContentStream
                ).mockResolvedValue(streamWithNoFinish);
        ...
        it('should succeed if the stream ends with an invalid part but has a finishReason and contained a valid part', async () => {
            ...
            vi.mocked(
                mockContentGenerator.generateContentStream
            ).mockResolvedValue(streamWithInvalidEnd);
        ...
        it('should consolidate subsequent text chunks after receiving an empty text chunk', async () => {
            ...
            vi.mocked(
                mockContentGenerator.generateContentStream
            ).mockResolvedValue(multiChunkStream);
        ...
        it('should preserve text parts that stream in the same chunk as a thought', async () => {
            ...
            vi.mocked(
                mockContentGenerator.generateContentStream
            ).mockResolvedValue(mixedContentStream);
        ...
        it('should throw an error when a tool call is followed by an empty stream response', async () => {
            ...
            try {
                ...
                const initialHistory: Content[] = [
                    {
                        role: 'user',
                        parts: [
                            { text: 'Find a good Italian restaurant for me.' }
                        ]
                    },
                    {
                        role: 'model',
                ...
                vi.mocked(
                    mockContentGenerator.generateContentStream
                ).mockResolvedValue(emptyStreamResponse);
        ...
        it('should throw InvalidStreamError when no tool call and no finish reason', async () => {
            ...
            try {
                ...
                vi.mocked(
                    mockContentGenerator.generateContentStream
                ).mockResolvedValue(streamWithoutFinishReason);
        ...
        it('should throw InvalidStreamError when no tool call and empty response text', async () => {
            ...
            try {
                ...
                vi.mocked(
                    mockContentGenerator.generateContentStream
                ).mockResolvedValue(streamWithEmptyResponse);
        ...
        it('should not lose finish reason when last chunk only has usage metadata', async () => {
            ...
            vi.mocked(
                mockContentGenerator.generateContentStream
            ).mockResolvedValue(streamWithTrailingUsageOnlyChunk);
    ...
    describe('sendMessageStream with retries', () => {
        it('should retry on invalid content, succeed, and report metrics', async () => {
            ...
            try {
                ...
                expect(
                    chunks.some((c) => c.type === StreamEventType.RETRY)
                ).toBe(true);
                ...
                expect(
                    chunks.some(
                        (c) =>
                            ...
                            c.value.candidates?.[0]?.content?.parts?.[0]
                                ?.text === 'Successful response'
                    )
                ).toBe(true);
                ...
                expect(
                    events.some(
                        (e) =>
                            ...
                            e.value.candidates?.[0]?.content?.parts?.[0]
                                ?.text === 'Recovered after empty stream'
                    )
                ).toBe(true);
            } finally {
                ...
                const successStream = (async function* () {
                    yield {
                        candidates: [
                            {
                                content: {
                                    parts: [{ text: 'Success after TPM retry' }]
                                },
                                finishReason: 'STOP'
                            }
                        ]
                ...
                const successStream = (async function* () {
                    yield {
                        candidates: [
                            {
                                content: {
                                    parts: [{ text: 'Success after GLM retry' }]
                                },
                                finishReason: 'STOP'
                            }
                        ]
    ...
    it('should correctly retry and append to an existing history mid-conversation', async () => {
        ...
        if (!turn1?.parts?.[0] || !('text' in turn1.parts[0])) {
            throw new Error(
                'Test setup error: First turn is not a valid text part.'
            );
        }
        ...
        if (!turn3?.parts?.[0] || !('text' in turn3.parts[0])) {
            throw new Error(
                'Test setup error: Third turn is not a valid text part.'
            );
        }
                            ...
                            candidates: [
                                {
                                    content: {
                                        parts: [
                                            {
                                                text: 'Successful response after empty'
                                            }
                                        ]
                                    },
                                    finishReason: 'STOP'
                        ...
                        candidates: [
                            {
                                content: {
                                    parts: [
                                        {
                                            text: 'This valid part should be discarded'
                                        }
                                    ]
                                }
                            }
