...
export class ContentGenerationPipeline {
    ...
    private async *processStreamWithLogging(
        ...
        try {
            ...
            for await (const chunk of stream) {
                ...
                if (
                    (chunk.choices?.[0]?.finish_reason as string) ===
                    'error_finish'
                ) {
                    const errorContent =
                ...
                const response =
                    this.converter.convertOpenAIChunkToGemini(chunk);
    ...
    private handleChunkMerging(
        ...
        if (isFinishChunk) {
            if (hasPendingFinish) {
                ...
                const lastResponse =
                    collectedGeminiResponses[
                        collectedGeminiResponses.length - 1
                    ];
                if (response.usageMetadata) {
    ...
    private buildGenerateContentConfig(
        ...
        const getParameterValue = <T>(
            ...
            const configValue = configSamplingParams?.[configKey] as
                | T
                | undefined;
