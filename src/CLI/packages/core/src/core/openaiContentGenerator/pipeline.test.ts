...
describe('ContentGenerationPipeline', () => {
    ...
    describe('execute', () => {
        it('should successfully execute non-streaming request', async () => {
            ...
            (
                mockConverter.convertGeminiRequestToOpenAI as Mock
            ).mockReturnValue(mockMessages);
            (
                mockConverter.convertOpenAIResponseToGemini as Mock
            ).mockReturnValue(mockGeminiResponse);
            ...
            ).toHaveBeenCalledWith(mockOpenAIResponse);
        });
        ...
        it('should handle tools in request', async () => {
            ...
            (
                mockConverter.convertGeminiToolsToOpenAI as Mock
        ...
        it('should handle errors and log them', async () => {
            ...
            // Act & Assert
            await expect(
                pipeline.execute(request, userPromptId)
        ...
        it('should pass abort signal to OpenAI client when provided', async () => {
            ...
            (
                mockConverter.convertOpenAIResponseToGemini as Mock
            ).mockReturnValue(new GenerateContentResponse());
            (mockClient.chat.completions.create as Mock).mockResolvedValue({
    ...
    describe('executeStream', () => {
        ...
        it('should handle streaming errors and reset tool calls', async () => {
            ...
            expect(mockConverter.resetStreamingToolCalls).toHaveBeenCalledTimes(
                2
        ...
        it('should merge finishReason and usageMetadata from separate chunks', async () => {
            ...
            expect(lastResult.candidates?.[0]?.finishReason).toBe(
                FinishReason.STOP
            );
        ...
        it('should handle providers that send zero usage in finish chunk (like modelscope)', async () => {
            ...
            const mockFinishResponseWithZeroUsage =
                new GenerateContentResponse();
        ...
        it('should not duplicate function calls when trailing chunks arrive after finish+usage merge', async () => {
            ...
            const mockChunk1 = {
                ...
                choices: [
                    {
                        delta: { content: 'I will create a todo' },
                        finish_reason: null
                    }
                ]
            } as OpenAI.Chat.ChatCompletionChunk;
            ...
            expect(
                mergedResult.candidates?.[0]?.content?.parts?.[0]?.functionCall
                    ?.name
            ).toBe('todoWrite');
    ...
    describe('buildRequest', () => {
        it('should build request with sampling parameters', async () => {
            ...
            (
                mockConverter.convertOpenAIResponseToGemini as Mock
            ).mockReturnValue(mockOpenAIResponse);
            (mockClient.chat.completions.create as Mock).mockResolvedValue({
                id: 'test',
                choices: [{ message: { content: 'response' } }]
            });
