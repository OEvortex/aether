...
describe('Gemini Client (client.ts)', () => {
    ...
    beforeEach(async () => {
        ...
        mockConfig = {
            ...
            getModelRouterService: vi.fn().mockReturnValue({
                route: vi
                    .fn()
                    .mockResolvedValue({ model: 'default-routed-model' })
            }),
    ...
    describe('resetChat', () => {
        it('should create a new chat session, clearing the old history', async () => {
            ...
            expect(JSON.stringify(newHistory)).not.toContain(
                'some old message'
            );
        });
    });
    ...
    describe('tryCompressChat', () => {
        ...
        function setup({
            ...
            ).mockReturnValue(originalTokenCount);
        ...
        describe('when compression inflates the token count', () => {
            it('allows compression to be forced/manual after a failure', async () => {
                ...
                const { client, estimatedNewTokenCount: inflatedTokenCount } =
                    setup({
            ...
            it('does not manipulate the source chat', async () => {
                // newTokenCount = 100 - (1010 - 1000) + 200 = 100 - 10 + 200 = 290 > 100 (inflation)
                const longSummary = 'long summary '.repeat(100);
                const { client, mockOriginalChat, estimatedNewTokenCount } =
                    setup({
                        originalTokenCount: 100,
                        summaryText: longSummary,
                        compressionInputTokenCount: 1010,
        ...
        it('logs a telemetry event when compressing', async () => {
            const { logChatCompression } = await import(
        ...
        it('should trigger summarization if token count is above threshold with contextPercentageThreshold setting', async () => {
            ...
            // newTokenCount might be clamped to originalTokenCount due to tolerance logic
            expect(result.newTokenCount).toBeLessThanOrEqual(
                originalTokenCount
            );
        ...
        describe('IDE context with pending tool calls', () => {
            ...
            beforeEach(() => {
                ...
                vi.mocked(ideContextStore.get).mockReturnValue({
                    workspaceState: {
                        openFiles: [
                            { path: '/path/to/file.ts', timestamp: Date.now() }
                        ]
                    }
                });
            });
            ...
            it('should send the latest IDE context on the next message after a skipped context', async () => {
                ...
                const initialIdeContext = {
                    workspaceState: {
                        openFiles: [
                            { path: '/path/to/fileA.ts', timestamp: Date.now() }
                        ]
                    }
                };
                vi.mocked(ideContextStore.get).mockReturnValue(
                ...
                const historyAfterToolResponse: Content[] = [
                    ...
                    {
                        role: 'model',
                        parts: [{ text: 'The tool ran successfully.' }]
                    }
                ];
                vi.mocked(mockChat.getHistory!).mockReturnValue(
                ...
                const newIdeContext = {
                    workspaceState: {
                        openFiles: [
                            { path: '/path/to/fileB.ts', timestamp: Date.now() }
                        ]
                    }
                };
                ...
                const addHistoryCalls = vi.mocked(mockChat.addHistory!).mock
                    .calls;
            ...
            it('should send a context DELTA on the next message after a skipped context', async () => {
                ...
                const initialCall = vi.mocked(mockChat.addHistory!).mock
                    .calls[0][0];
                ...
                const finalCall = vi.mocked(mockChat.addHistory!).mock
                    .calls[0][0];
                expect(JSON.stringify(finalCall)).toContain(
                    'summary of changes'
                );
        ...
        describe('hooks fast-path optimization', () => {
            ...
            it('should not skip hooks when hasHooksForEvent returns true', async () => {
                const mockMessageBus = {
                    request: vi
                        .fn()
                        .mockResolvedValue({ modifiedPrompt: undefined }),
                    response: vi.fn()
                };
                vi.mocked(mockConfig.getDisableAllHooks).mockReturnValue(false);
                vi.mocked(mockConfig.getMessageBus).mockReturnValue(
