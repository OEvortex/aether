...
const memoryToolSchemaData = {
    ...
    parametersJsonSchema: {
        ...
        properties: {
            ...
            scope: {
                type: 'string',
                description: 'Where to save the memory: "global" saves to user-level ~/.aether/AGENTS.md (shared across all projects), "project" saves to current project\'s AGENTS.md (project-specific). If not specified, will prompt user to choose.',
                enum: ['global', 'project']
            }
        },
        required: ['fact']
...
export class MemoryTool extends BaseDeclarativeTool {
    ...
    static async performAddMemoryEntry(text, memoryFilePath, fsAdapter) {
        try {
            await fsAdapter.mkdir(path.dirname(memoryFilePath), {
                recursive: true
            });
            let currentContent = '';
            try {
