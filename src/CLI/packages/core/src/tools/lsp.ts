...
class LspToolInvocation extends BaseToolInvocation<LspToolParams, ToolResult> {
    ...
    private async executeImplementations(
        client: LspClient
    ): Promise<ToolResult> {
        const target = this.resolveLocationTarget();
        if ('error' in target) {
    ...
    private async executeDocumentSymbols(
        ...
        if (!uri) {
            const message =
                'A valid filePath is required for document symbols.';
            return { llmContent: message, returnDisplay: message };
        }
    ...
    private async executeWorkspaceDiagnostics(
        ...
        for (const fileDiag of fileDiagnostics) {
            const fileLabel = this.formatUriForDisplay(
                fileDiag.uri,
                workspaceRoot
            );
    ...
    private async executeCodeActions(client: LspClient): Promise<ToolResult> {
        ...
        const lines = actions.slice(0, limit).map((action, index) => {
            ...
            const serverSuffix = action.serverName
                ? ` [${action.serverName}]`
                : '';
    ...
    private formatCallRanges(ranges: LspRange[]): string {
        ...
        const formatted = ranges.map((range) =>
            this.formatPosition(range.start)
        );
        const maxShown = 3;
            ...
            Kind.Other,
            {
                ...
                properties: {
                    ...
                    line: {
                        type: 'number',
                        description:
                            '1-based line number for the target location.'
                    },
                    character: {
                        type: 'number',
                        description:
                    ...
                    endLine: {
                        type: 'number',
                        description:
                            '1-based end line number for range-based operations.'
                    },
                    endCharacter: {
                        type: 'number',
                        description:
                            '1-based end character for range-based operations.'
                    },
                    includeDeclaration: {
                        type: 'boolean',
                        description:
                    ...
                    callHierarchyItem: {
                        ...
                        description:
                            'Call hierarchy item for incoming/outgoing calls.'
                    },
                    serverName: {
                        type: 'string',
