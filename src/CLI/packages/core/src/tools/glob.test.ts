...
describe('GlobTool', () => {
    ...
    beforeEach(async () => {
        ...
        await fs.writeFile(
            path.join(tempRootDir, 'older.sortme'),
            'older_content'
        );
        // Ensure a noticeable difference in modification time
        await new Promise((resolve) => setTimeout(resolve, 50));
        await fs.writeFile(
            path.join(tempRootDir, 'newer.sortme'),
            'newer_content'
        );
    ...
    describe('validateToolParams', () => {
        ...
        it("should return error if search path resolves outside the tool's root directory", () => {
            ...
            expect(
                specificGlobTool.validateToolParams(paramsOutside)
            ).toBeNull();
        });
    ...
    describe('ignore file handling', () => {
        ...
        it('should respect .gitignore when searching a subdirectory (path option)', async () => {
            ...
            await fs.writeFile(
                path.join(tempRootDir, 'sub', 'visible.txt'),
                'ok'
            );
            await fs.writeFile(
