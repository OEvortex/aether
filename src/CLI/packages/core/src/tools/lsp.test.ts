...
import { type LspOperation, LspTool, type LspToolParams } from './lsp.js';
...
describe('LspTool', () => {
    describe('validateToolParams', () => {
        ...
        describe('location-based operations', () => {
            ...
            it.each(
                locationOperations
            )('requires filePath for %s operation', (operation) => {
            ...
            it.each(
                locationOperations
            )('requires line for %s operation', (operation) => {
                const result = tool.validateToolParams({
                    operation,
                    filePath: 'src/app.ts'
                } as LspToolParams);
            ...
            it.each(
                locationOperations
            )('passes validation with valid params for %s', (operation) => {
                const result = tool.validateToolParams({
                    operation,
                    filePath: 'src/app.ts',
                    line: 10,
    ...
    describe('execute', () => {
        ...
        describe('incomingCalls operation', () => {
            it('dispatches to incomingCalls and formats results', async () => {
                ...
                (client.incomingCalls as Mock).mockResolvedValue([
                    incomingCall
                ]);
        ...
        describe('outgoingCalls operation', () => {
            it('dispatches to outgoingCalls and formats results', async () => {
                ...
                (client.outgoingCalls as Mock).mockResolvedValue([
                    outgoingCall
                ]);
        ...
        describe('error handling', () => {
            ...
            it('handles hover operation errors', async () => {
                const client = createMockClient();
                const tool = createTool(client);
                (client.hover as Mock).mockRejectedValue(
    ...
    describe('schema compatibility with Claude Code', () => {
        ...
        describe('schema definitions deep validation', () => {
            ...
            it('callHierarchyItem uses $ref for range fields', () => {
                ...
                const itemDef = schema.definitions?.LspCallHierarchyItem;
                expect(itemDef?.properties?.range?.$ref).toBe(
