            ...
            if (wwwAuthenticate) {
                    ...
                    if (credentials) {
                        ...
                        if (accessToken) {
                            ...
                            const oauthTransport =
                                await createTransportWithOAuth(
                                    mcpServerName,
                                    mcpServerConfig,
                                    accessToken
                                );
                ...
                if (hasNetworkTransport(mcpServerConfig)) {
                    ...
                    try {
                        ...
                        const oauthConfig =
                            await OAuthUtils.discoverOAuthConfig(baseUrl);
                        if (oauthConfig) {
                            debugLogger.info(
                            ...
                            if (credentials) {
                                ...
                                const accessToken =
                                    await authProvider.getValidToken(
                                        mcpServerName,
                                        {
                                            // Pass client ID if available
                                            clientId: credentials.clientId
...
export function isEnabled(
    ...
    return (
        ...
        includeTools.some(
            (tool) =>
                tool === funcDecl.name || tool.startsWith(`${funcDecl.name}(`)
        )
    );
}
