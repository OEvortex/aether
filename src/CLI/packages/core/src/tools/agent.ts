    ...
    async refreshSubagents(): Promise<void> {
        try {
            this.availableSubagents =
                await this.subagentManager.listSubagents();
...
class AgentToolInvocation extends BaseToolInvocation<AgentParams, ToolResult> {
            ...
            (...args: unknown[]) => {
                ...
                const details: ToolCallConfirmationDetails = {
                    ...
                    onConfirm: async (
                        ...
                        pendingConfirmationCallId = undefined;
                        const proceedOutcomes =
                            new Set<ToolConfirmationOutcome>([
    ...
    async execute(
        ...
        try {
            ...
            debugLogger.error(
                `[AgentTool] Error running subagent: ${errorMessage}`
            );
