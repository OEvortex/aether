...
export class AgentInteractive {
    ...
    private async runOneRound(message: string): Promise<void> {
        ...
        this.masterAbortController.signal.addEventListener(
            'abort',
            onMasterAbort
        );
        if (this.masterAbortController.signal.aborted) {
        ...
        try {
            ...
            this.addMessage('info', errorMessage, {
    ...
    private setupEventListeners(): void {
        ...
        emitter.on(AgentEventType.ROUND_TEXT, (event: AgentRoundTextEvent) => {
            if (event.thoughtText) {
                this.addMessage('assistant', event.thoughtText, {
                    thought: true
                });
            }
            if (event.text) {
...
function terminateModeMessage(
    ...
    switch (mode) {
        ...
        case AgentTerminateMode.TIMEOUT:
            return {
                text: 'Agent stopped: time limit reached.',
                level: 'warning'
            };
