...
export class AgentCore {
    ...
    async runReasoningLoop(chat, initialMessages, toolsList, abortController, options) {
        ...
        while (true) {
            ...
            if (options?.maxTimeMinutes &&
                durationMin >= options.maxTimeMinutes) {
            ...
            const responseStream = await chat.sendMessageStream(this.modelConfig.model ||
                ...
                DEFAULT_AETHER_MODEL, messageParams, promptId);
        ...
        const scheduler = new CoreToolScheduler({
            ...
            onAllToolCallsComplete: async (completedCalls) => {
                for (const call of completedCalls) {
                    ...
                    if (respParts) {
                        const parts = Array.isArray(respParts)
                            ? respParts
                            : [respParts];
    ...
    getStatistics() {
        ...
        const successRate = total > 0
            ? (this.executionStats.successfulToolCalls / total) * 100
            : 0;
        return {
            ...this.executionStats,
            successRate,
...
Important Rules:
 ...
 - When the task is complete, return the final result as a normal model response (not a tool call) and stop.`;
    ...
    recordTokenUsage(usage, turnCounter, roundStreamStart) {
        ...
        const contextTok = Number.isFinite(totalTok) && totalTok > 0 ? totalTok : inTok;
        ...
        if (Number.isFinite(inTok) ||
            ...
            this.stats.recordTokens(Number.isFinite(inTok) ? inTok : 0, Number.isFinite(outTok) ? outTok : 0, Number.isFinite(thoughtTok) ? thoughtTok : 0, Number.isFinite(cachedTok) ? cachedTok : 0, Number.isFinite(totalTok) ? totalTok : 0);
