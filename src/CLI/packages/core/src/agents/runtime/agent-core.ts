...
export class AgentCore {
        ...
        if (this.toolConfig) {
            ...
            if (hasWildcard || asStrings.length === 0) {
                toolsList.push(
                    ...toolRegistry
                        ...
                        .filter(
                            (t) =>
                                !(t.name && excludedFromSubagents.has(t.name))
                        )
                );
            } else {
                toolsList.push(
                    ...toolRegistry.getFunctionDeclarationsFiltered(
                        asStrings.filter(
                            (name) => !excludedFromSubagents.has(name)
                        )
                    )
                );
            }
            ...
            toolsList.push(
                ...toolRegistry
                    .getFunctionDeclarations()
                    .filter(
                        (t) => !(t.name && excludedFromSubagents.has(t.name))
                    )
            );
        }
            ...
            onToolCallsUpdate: (calls: ToolCall[]) => {
                for (const call of calls) {
                    ...
                    try {
                        ...
                        const { onConfirm: _onConfirm, ...rest } =
                            confirmationDetails;
                        this.eventEmitter?.emit(
    ...
    getStatistics(): {
        ...
        return {
            ...this.executionStats,
            successRate,
            toolUsage: Array.from(this.toolUsage.entries()).map(
                ([name, v]) => ({
                    name,
                    ...v
                })
