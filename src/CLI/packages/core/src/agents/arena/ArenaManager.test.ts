...
import { ARENA_MAX_AGENTS, ArenaSessionStatus } from './types.js';
...
describe('ArenaManager', () => {
    ...
    describe('constructor', () => {
        it('should create an ArenaManager instance', () => {
            ...
            expect(manager.getSessionStatus()).toBe(
                ArenaSessionStatus.INITIALIZING
            );
        });
    ...
    describe('backend initialization', () => {
        it('should emit SESSION_UPDATE with type warning when backend detection returns warning', async () => {
            ...
            manager
                .getEventEmitter()
                .on(ArenaEventType.SESSION_UPDATE, (event) => {
                    updates.push({
                        type: event.type,
            ...
            expect(warningUpdate?.message).toContain(
                'fallback to tmux backend'
            );
        ...
        it('should emit SESSION_ERROR and mark FAILED when backend init fails', async () => {
            ...
            manager
                .getEventEmitter()
                .on(ArenaEventType.SESSION_ERROR, (event) => {
            ...
            await expect(
                manager.start(createValidStartOptions())
