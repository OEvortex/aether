...
export class ArenaManager {
    ...
    async start(options: ArenaStartOptions): Promise<ArenaSessionResult> {
        ...
        this.arenaConfig = {
            ...
            timeoutSeconds:
                options.timeoutSeconds ?? arenaSettings?.timeoutSeconds,
        ...
        try {
            ...
            const worktreeInfo = Array.from(this.agents.values())
                ...
                .join('\n');
            this.emitProgress(
                `Environment ready. Agent worktrees:\n${worktreeInfo}`
            );
    ...
    private async initializeBackend(displayMode?: DisplayMode): Promise<void> {
        const { backend, warning } = await detectBackend(
            displayMode,
            this.config
        );
        await backend.init();
    ...
    private async runAgents(): Promise<void> {
        ...
        if (!allSettled) {
            debugLogger.info(
                'Arena session timed out, stopping remaining agents'
            );
            this.sessionStatus = ArenaSessionStatus.CANCELLED;
    ...
    private updateAgentStatus(
        ...
        if (
            ...
            if (options?.roundCancelledByUser) {
                this.emitProgress(
                    `Agent ${label} is cancelled by user.`,
                    'warning'
                );
            } else {
                this.emitProgress(
                    `Agent ${label} finished initial task.`,
                    'success'
                );
            }
        }
        ...
        if (this.sessionStatus === ArenaSessionStatus.IDLE) {
            if (
                ...
                this.emitProgress(
                    `Agent ${label} is working on a follow-up task…`
                );
            } else if (
                previousStatus === AgentStatus.RUNNING &&
                newStatus === AgentStatus.IDLE
        ...
        for (const agent of this.agents.values()) {
            ...
            const onStatusChange = (event: AgentStatusChangeEvent) => {
                ...
                this.flushInProcessStatusFiles().catch((err) =>
                    debugLogger.error(
                        'Failed to flush in-process status files:',
                        err
                    )
                );
            };
    ...
    private async pollAgentStatuses(): Promise<void> {
        ...
        for (const agent of this.agents.values()) {
            ...
            try {
                ...
                this.callbacks.onAgentStatsUpdate?.(
                    agent.agentId,
                    statusFile.stats
                );
            } catch (error: unknown) {
