...
export class TmuxBackend implements Backend {
    ...
    getAttachHint(): string | null {
        ...
        const server =
            this.serverName ?? `${TMUX_SERVER_PREFIX}-${process.pid}`;
    ...
    private resolveTmuxOptions(config: AgentSpawnConfig): ResolvedTmuxOptions {
        ...
        return {
            serverName:
                opts.serverName ?? `${TMUX_SERVER_PREFIX}-${process.pid}`,
            ...
            paneBorderFormat:
                opts.paneBorderFormat ?? DEFAULT_PANE_BORDER_FORMAT,
            paneBorderStatus:
            ...
            firstSplitPercent:
                opts.firstSplitPercent ?? DEFAULT_FIRST_SPLIT_PERCENT
        };
    }
    ...
    private async ensureExternalSession(
        ...
        const sessionExists = await tmuxHasSession(
            this.sessionName,
            serverName
        );
        ...
        if (!this.mainPaneId) {
            this.mainPaneId = await tmuxGetFirstPaneId(
                this.windowTarget,
                serverName
            );
        }
    ...
    private buildShellCommand(config: AgentSpawnConfig): string {
        ...
        debugLogger.info(
            `[buildShellCommand] full shell command: ${fullCommand}`
        );
        return fullCommand;
    }
