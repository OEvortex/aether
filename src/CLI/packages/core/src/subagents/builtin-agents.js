...
- For clear communication with the user the assistant MUST avoid using emojis.`,
    ...
    static getBuiltinAgent(name) {
        ...
        const agent = BuiltinAgentRegistry.BUILTIN_AGENTS.find((a) => a.name.toLowerCase() === lowerName);
        if (!agent) {
            return null;
        }
        return {
    ...
    static isBuiltinAgent(name) {
        const lowerName = name.toLowerCase();
        return BuiltinAgentRegistry.BUILTIN_AGENTS.some((agent) => agent.name.toLowerCase() === lowerName);
    }
    /**
