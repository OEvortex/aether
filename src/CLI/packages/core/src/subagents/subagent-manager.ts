...
export class SubagentManager {
    ...
    private notifyChangeListeners(): void {
        for (const listener of this.changeListeners) {
            try {
                ...
                debugLogger.warn(
                    'Subagent change listener threw an error:',
                    error
                );
            }
        }
    ...
    async createSubagent(
        ...
        const filePath =
            options.customPath ||
            this.getSubagentPath(config.name, options.level);
    ...
    async loadSubagent(
        ...
        if (level) {
            ...
            if (level === 'session') {
                const sessionSubagents =
                    this.subagentsCache?.get('session') || [];
                return (
        ...
        // Try project level
        const projectConfig = await this.findSubagentByNameAtLevel(
            name,
            'project'
        );
        if (projectConfig) {
    ...
    async deleteSubagent(
        ...
        for (const currentLevel of levelsToCheck) {
            ...
            const config = await this.findSubagentByNameAtLevel(
                name,
                currentLevel
            );
            ...
            for (const level of levelsToCheck) {
                ...
                for (const subagent of levelSubagents) {
                    ...
                    if (
                        options.hasTool &&
                        (!subagent.tools ||
                            !subagent.tools.includes(options.hasTool))
                    ) {
                        continue;
                    }
    ...
    private async listSubagentsAtLevel(
        ...
        try {
            ...
            for (const file of files) {
                ...
                try {
                    const config = await this.parseSubagentFile(
                        filePath,
                        level
                    );
