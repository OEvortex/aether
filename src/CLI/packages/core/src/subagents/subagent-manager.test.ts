...
You are a helpful assistant.`;
            ...
            const config = await manager.loadSubagent(
                'specific-agent',
                'project'
            );
    ...
    describe('updateSubagent', () => {
        ...
        it('should throw error on write failure', async () => {
            ...
            await expect(
                manager.updateSubagent('test-agent', {})
            ).rejects.toThrow(/Failed to update subagent file/);
        });
    });
    ...
    describe('deleteSubagent', () => {
        ...
        it('should succeed if deleted from at least one level', async () => {
            ...
            await expect(
                manager.deleteSubagent('test-agent')
...
System prompt 3`);
        ...
        it('should filter by level', async () => {
            ...
            expect(projectSubagents.every((s) => s.level === 'project')).toBe(
                true
            );
        });
    ...
    describe('Runtime Configuration Methods', () => {
        describe('convertToRuntimeConfig', () => {
            it('should convert basic configuration', () => {
                const runtimeConfig =
                    manager.convertToRuntimeConfig(validConfig);
            ...
            it('should set modelConfig.model from model selector and merge run configurations', () => {
                ...
                const runtimeConfig =
                    manager.convertToRuntimeConfig(configWithCustom);
