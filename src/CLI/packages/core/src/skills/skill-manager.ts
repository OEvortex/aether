...
export class SkillManager {
    ...
    async loadSkill(
        ...
        // Try extension level
        const extensionSkill = await this.findSkillByNameAtLevel(
            name,
            'extension'
        );
        if (extensionSkill) {
    ...
    async refreshCache(): Promise<void> {
        ...
        for (const level of levels) {
            ...
            debugLogger.debug(
                `Loaded ${levelSkills.length} ${level} level skills`
            );
        }
        ...
        try {
            ...
            for (const entry of entries) {
                ...
                if (!isDirectory && !isSymlink) {
                    debugLogger.warn(
                        `Skipping non-directory entry: ${entry.name}`
                    );
                    continue;
                }
        ...
        for (const watchPath of watchTargets) {
            ...
            try {
                const watcher = watchFs(watchPath, {
                    ...
                    .on('error', (error) => {
                        debugLogger.warn(
                            `Skills watcher error for ${watchPath}:`,
                            error
                        );
                    });
