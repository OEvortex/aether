...
`;
        ...
        it('should determine level from file path', () => {
            const projectPath =
                '/test/project/.aether/skills/test-skill/SKILL.md';
...
Skill 3 content`);
        ...
        it('should filter by level', async () => {
            ...
            expect(projectSkills.every((s) => s.level === 'project')).toBe(
                true
            );
        });
        ...
        it('should deduplicate same-name skills across provider dirs within a level', async () => {
            ...
            const projectQwenDir = path.join(
                '/test/project',
                '.aether',
                'skills'
            );
            ...
            vi.mocked(fs.readFile).mockImplementation((filePath) => {
                const pathStr = String(filePath);
                if (
                    pathStr.includes('.aether') &&
                    pathStr.includes('shared-skill')
                ) {
                    return Promise.resolve(
    ...
    describe('getSkillsBaseDirs', () => {
        it('should return all project-level base dirs', () => {
            ...
            expect(baseDirs).toContain(
                path.join('/test/project', '.aether', 'skills')
            );
            expect(baseDirs).toContain(
        ...
        it('should return all user-level base dirs', () => {
            ...
            expect(baseDirs).toHaveLength(2);
            expect(baseDirs).toContain(
                path.join('/home/user', '.aether', 'skills')
            );
            expect(baseDirs).toContain(
                path.join('/home/user', '.agents', 'skills')
            );
        });
    ...
    describe('bundled skills', () => {
        ...
        const emptyDir = [] as unknown as Awaited<
        ...
        function mockReaddirForLevels(levels: Set<string>) {
            vi.mocked(fs.readdir).mockImplementation((dirPath) => {
                ...
                const isUser =
                    pathStr.includes(userDirSegment) &&
                    pathStr.startsWith(userPrefix);
