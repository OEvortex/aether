...
export class IdeClient {
        ...
        if (argsStr) {
            try {
                ...
                if (Array.isArray(parsedArgs)) {
                    ...
                    debugLogger.error('AETHER_CODE_IDE_SERVER_STDIO_ARGS must be a JSON array string.');
                }
            }
            catch (e) {
                debugLogger.error('Failed to parse AETHER_CODE_IDE_SERVER_STDIO_ARGS:', e);
            }
        }
        return { command, args };
    }
    ...
    async getLegacyConnectionConfig(portFromEnv) {
        if (this.ideProcessInfo) {
            try {
                const portFile = path.join(os.tmpdir(), `aether-ide-server-${this.ideProcessInfo.pid}.json`);
        ...
        if (portFromEnv) {
            try {
                const portFile = path.join(os.tmpdir(), `aether-ide-server-${portFromEnv}.json`);
                const portFileContents = await fs.promises.readFile(portFile, 'utf8');
                return JSON.parse(portFileContents);
            }
            catch (_) {
