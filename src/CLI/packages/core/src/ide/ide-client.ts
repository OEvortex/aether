...
export class IdeClient {
    ...
    private diffResponses = new Map<
        string,
        (result: DiffUpdateResult) => void
    >();
        ...
        if (!IdeClient.instancePromise) {
            IdeClient.instancePromise = (async () => {
                ...
                client.connectionConfig =
                    await client.getConnectionConfigFromFile();
    ...
    async connect(): Promise<void> {
        ...
        if (stdioConfigFromEnv) {
            const connected =
                await this.establishStdioConnection(stdioConfigFromEnv);
            if (connected) {
                return;
            }
        }
    ...
    async openDiff(
        ...
        const promise = new Promise<DiffUpdateResult>((resolve, reject) => {
            ...
            this.client
                ...
                .catch((err) => {
                    debugLogger.debug(
                        `Request for openDiff ${filePath} failed:`,
                        err
                    );
                    this.diffResponses.delete(filePath);
                    reject(err);
    ...
    private async discoverTools(): Promise<void> {
        ...
        try {
            ...
            if (
                ...
                debugLogger.error(
                    `Error discovering tools from IDE: ${error.message}`
                );
            } else {
    ...
    > {
        ...
        if (portFromEnv) {
            try {
                ...
                const lockFileContents = await fs.promises.readFile(
                    lockFile,
                    'utf8'
                );
        ...
        return configs.find(
            (config) =>
                ...
                IdeClient.validateWorkspacePath(config.workspacePath, cwd)
                    .isValid
        );
    }
    ...
    protected async getAllConnectionConfigs(
            ...
            lockFiles.map(async (file) => {
                ...
                try {
                    ...
                    try {
                        ...
                        debugLogger.debug(
                            'Failed to parse JSON from lock file: ',
                            error
                        );
                        return undefined;
                    }
                    ...
                    debugLogger.debug(
                        'Failed to read/stat IDE lock file:',
                        error
                    );
                    return undefined;
                }
        ...
        const staleCount = activeLockFiles.filter(
            ({ isStale }) => isStale
        ).length;
        if (staleCount > 0) {
            debugLogger.debug(
    ...
    private createProxyAwareFetch(ideHost: string) {
        ...
        return async (
            ...
            return new Response(
                response.body as ReadableStream<unknown> | null,
                {
    ...
    private async tryHttpConnect(port: string, host: string): Promise<boolean> {
        ...
        try {
            ...
            debugLogger.debug(
                `HTTP connection to ${host}:${port} failed:`,
                error
            );
            if (transport) {
                try {
