...
import { SharedTokenManager, TokenError, TokenManagerError } from './aetherSharedTokenManager.js';
 ...
 */
export class AetherOAuth2Client {
    credentials = {};
    sharedManager;
    constructor() {
...
export var AetherOAuth2Event;
(function (AetherOAuth2Event) {
    AetherOAuth2Event["AuthUri"] = "auth-uri";
    AetherOAuth2Event["AuthProgress"] = "auth-progress";
    AetherOAuth2Event["AuthCancel"] = "auth-cancel";
})(AetherOAuth2Event || (AetherOAuth2Event = {}));
...
export async function getaetherOAuthClient(config, options) {
    ...
    try {
        ...
        if (options?.requireCachedCredentials) {
            throw new Error('Aether OAuth credentials expired. Please use /auth to re-authenticate with aether-oauth.');
        }
        ...
        if (!result.success) {
            ...
            if (result.reason === 'timeout') {
                aetherOAuth2Events.emit(AetherOAuth2Event.AuthProgress, 'timeout', 'Authentication timed out. Please try again or select a different authentication method.');
            }
...
function showFallbackMessage(verificationUriComplete) {
    ...
    const wrapText = (text, width) => {
        ...
        for (const word of words) {
            if (currentLine.length + word.length + 1 <= width) {
                ...
                currentLine =
                    word.length > width ? word.substring(0, width) : word;
            }
        }
        if (currentLine) {
            lines.push(currentLine);
    ...
    process.stderr.write('| ' +
        waitingLine +
        ' '.repeat(contentWidth - waitingLine.length) +
        ' |\n');
    process.stderr.write(`${emptyLine}\n`);
...
async function authWithQwenDeviceFlow(client, config) {
    ...
    const launchBrowser = async (url) => {
        try {
            ...
            debugLogger.debug('Failed to open browser:', err instanceof Error ? err.message : 'Unknown error');
        }
    };
    try {
...
async function cacheAetherCredentials(credentials) {
 ...
 */
export async function clearAetherCredentials() {
    try {
        const filePath = getQwenCachedCredentialPath();
...
//# sourceMappingURL=aetherOAuth2.js.map