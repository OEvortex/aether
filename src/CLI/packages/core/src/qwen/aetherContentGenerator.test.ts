...
describe('AetherContentGenerator', () => {
    ...
    describe('Token Management and Refresh Logic', () => {
        ...
        it('should handle token refresh failure', async () => {
            ...
            await expect(
                aetherContentGenerator.generateContent(
                    request,
                    'test-prompt-id'
                )
            ).rejects.toThrow(
        ...
        it('should update endpoint when token is refreshed', async () => {
            ...
            await aetherContentGenerator.generateContent(
                request,
                'test-prompt-id'
            );
    ...
    describe('Endpoint URL Normalization', () => {
        it('should use default endpoint when no custom endpoint provided', async () => {
            ...
            parentPrototype.generateContent = vi
                .fn()
                .mockImplementation(function (this: AetherContentGenerator) {
                    capturedBaseURL = (
                        this as unknown as {
                            pipeline: { client: { baseURL: string } };
                        }
        ...
        it('should retry once on authentication errors', async () => {
            ...
            const mockGenerateContent = vi
                ...
                .mockResolvedValueOnce(
                    createMockResponse('Success after retry')
                );
            ...
            vi.mocked(mockQwenClient.getAccessToken).mockImplementation(
                async () => {
    ...
    describe('Token State Management', () => {
        it('should cache and return current token', () => {
            ...
            expect(aetherContentGenerator.getCurrentToken()).toBe(
                'cached-token'
            );
        });
    ...
    describe('Edge Cases and Error Conditions', () => {
        it('should handle token retrieval with warning when SharedTokenManager fails', async () => {
            const mockTokenManager = {
                getValidCredentials: vi
                    .fn()
                    .mockRejectedValue(
                        new Error('Internal token manager error')
                    )
            };
        ...
        it('should handle method types with token failure (except countTokens)', async () => {
            ...
            await expect(
                newGenerator.embedContent(embedRequest)
            ).rejects.toThrow('Failed to obtain valid Qwen access token');
