...
import { CredentialsClearRequiredError, isErrorResponse } from './aetherOAuth2.js';
...
export class SharedTokenManager {
     ...
     */
    async getValidCredentials(aetherClient, forceRefresh = false) {
        try {
            ...
            if (!currentRefreshPromise) {
                ...
                currentRefreshPromise = this.performTokenRefresh(aetherClient, forceRefresh);
     ...
     */
    async checkAndReloadIfNeeded(aetherClient) {
     ...
     */
    async performFileCheck(aetherClient, checkTime) {
     ...
     */
    async forceFileCheck(aetherClient) {
        try {
            const filePath = this.getCredentialFilePath();
     ...
     */
    async reloadCredentialsFromFile(aetherClient) {
        try {
            const filePath = this.getCredentialFilePath();
     ...
     */
    async performTokenRefresh(aetherClient, forceRefresh = false) {
...
//# sourceMappingURL=aetherSharedTokenManager.js.map