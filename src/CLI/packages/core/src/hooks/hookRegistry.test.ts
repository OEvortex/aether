...
import type { FeedbackEmitter, HookRegistryConfig } from './hookRegistry.js';
...
describe('HookRegistry', () => {
        ...
        it('should not process project hooks in untrusted folder', async () => {
            ...
            const hooksConfig = {
                [HookEventName.PreToolUse]: [
                    {
                        hooks: [
                            { type: HookType.Command, command: 'echo test' }
                        ]
                    }
                ]
            };
    ...
    describe('getHooksForEvent', () => {
        it('should return hooks for specific event', async () => {
            ...
            const preHooks = registry.getHooksForEvent(
                HookEventName.PreToolUse
            );
            expect(preHooks).toHaveLength(1);
            expect(preHooks[0].config.name).toBe('pre-hook');

            const postHooks = registry.getHooksForEvent(
                HookEventName.PostToolUse
            );
