...
export class AetherLogger {
    ...
    enqueueLogEvent(event: RumEvent): void {
        try {
            ...
            this.debugLogger.error(
                'AetherLogger: Failed to enqueue log event.',
                error
            );
        }
    }
    ...
    readSourceInfo(): string {
        try {
            ...
            if (fs.existsSync(sourceJsonPath)) {
                const sourceJsonContent = fs.readFileSync(
                    sourceJsonPath,
                    'utf8'
                );
            ...
            'session_start',
            {
                properties: {
                    ...
                    truncate_tool_output_threshold:
                        event.truncate_tool_output_threshold
                }
            }
        );
    ...
    logAuthEvent(event: AuthEvent): void {
        const rumEvent = this.createActionEvent('auth', 'auth', {
            properties: {
                ...
                error_type:
                    event.status !== 'success' ? event.status : undefined,
                error_message:
    ...
    private requeueFailedEvents(eventsToSend: RumEvent[]): void {
        ...
        const numEventsToRequeue = Math.min(
