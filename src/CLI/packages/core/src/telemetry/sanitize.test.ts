...
describe('sanitizeHookName', () => {
    ...
    it('should extract command name from full path on Unix systems', () => {
        ...
        expect(
            sanitizeHookName('/path/to/.gemini/hooks/check-secrets.sh')
    ...
    it('should extract command name from full path on Windows systems', () => {
        expect(sanitizeHookName('C:\\Windows\\System32\\cmd.exe')).toBe(
            'cmd.exe'
        );
        expect(
            sanitizeHookName('C:\\Users\\User\\Documents\\test.bat /c')
        ).toBe('test.bat');
    });
    ...
    it('should handle complex command lines', () => {
        ...
        expect(
            sanitizeHookName(
                'python /home/user/script.py --token=xyz --verbose'
            )
        ).toBe('python');
    });
