...
export class ModelsConfig {
    ...
    runtimeModelProvidersConfig;
    runtimeModelsDirty = true;
    ...
    requireCachedAetherCredentialsOnce = false;
    ...
    constructor(options = {}) {
        ...
        this.authTypeWasExplicitlyProvided =
            options.initialAuthType !== undefined;
    ...
    rollbackToStateSnapshot(snapshot) {
        ...
        this.strictModelProviderSelection =
            snapshot.strictModelProviderSelection;
        ...
        this.activeRuntimeModelSnapshotId =
            snapshot.activeRuntimeModelSnapshotId;
    }
    /**
    ...
    getAllConfiguredModels(authTypes) {
        const inputAuthTypes = authTypes && authTypes.length > 0
            ? authTypes
        ...
        try {
            ...
            if (runtimeModelSnapshot.apiKeyEnvKey) {
                this._generationConfig.apiKeyEnvKey =
                    runtimeModelSnapshot.apiKeyEnvKey;
            }
    ...
    reloadModelProvidersConfig(modelProvidersConfig) {
        ...
        this.modelRegistry.reloadModels(this.modelProvidersConfig, this.runtimeModelProvidersConfig);
    }
    /**
     * Refresh runtime-discovered models from provider templates.
     */
    async refreshRuntimeModelProviders(forceRefresh = false) {
        if (!forceRefresh && !this.runtimeModelsDirty) {
            return;
        }
        const discovered = await discoverRuntimeModelProviders(this.modelProvidersConfig, this.discoveryEnv, this._generationConfig.apiKey);
