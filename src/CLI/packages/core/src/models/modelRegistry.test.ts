...
import { AETHER_OAUTH_MODELS, ModelRegistry } from './modelRegistry.js';
...
describe('ModelRegistry', () => {
    describe('initialization', () => {
        it('should always include hard-coded aether-oauth models', () => {
            ...
            const aetherModels = registry.getModelsForAuthType(
        ...
        it('should initialize with empty config', () => {
            const registry = new ModelRegistry();
            expect(
                registry.getModelsForAuthType(AuthType.AETHER_OAUTH).length
            ).toBe(AETHER_OAUTH_MODELS.length);
            expect(
                registry.getModelsForAuthType(AuthType.USE_OPENAI).length
            ).toBe(0);
        });
        ...
        it('should initialize with custom models config', () => {
            ...
            const openaiModels = registry.getModelsForAuthType(
                AuthType.USE_OPENAI
            );
        ...
        it('should ignore aether-oauth models in config (hard-coded)', () => {
            ...
            expect(aetherModels.length).toBe(AETHER_OAUTH_MODELS.length);
            expect(
                aetherModels.find((m) => m.id === 'custom-aether')
            ).toBeUndefined();
        });
    });
    ...
    describe('getModelsForAuthType', () => {
        ...
        it('should return empty array for non-existent authType', () => {
            const models = registry.getModelsForAuthType(
    ...
    describe('getModel', () => {
        ...
        it('should preserve generationConfig without applying defaults', () => {
            ...
            expect(model?.generationConfig.samplingParams?.temperature).toBe(
                0.8
            );
            expect(model?.generationConfig.samplingParams?.max_tokens).toBe(
                4096
            );
            // No defaults are applied - only the configured values are present
            expect(
                model?.generationConfig.samplingParams?.top_p
            ).toBeUndefined();
    ...
    describe('hasModel', () => {
        ...
        it('should return false for non-existent authType', () => {
            expect(registry.hasModel(AuthType.USE_VERTEX_AI, 'gpt-4')).toBe(
                false
            );
        });
    });
    ...
    describe('authType key validation', () => {
        it('should accept valid authType keys', () => {
            ...
            const geminiModels = registry.getModelsForAuthType(
                AuthType.USE_GEMINI
            );
        ...
        it('should handle mixed valid and invalid keys', () => {
            ...
            expect(
                registry.getModelsForAuthType(AuthType.USE_GEMINI).length
            ).toBe(1);
        ...
        it('should work correctly with getModelsForAuthType after validation', () => {
            ...
            expect(
                models.find((m) => m.id === 'invalid-model')
            ).toBeUndefined();
        });
    });
    ...
    describe('duplicate model id handling', () => {
        ...
        it('should handle multiple duplicate ids in same authType', () => {
            ...
            expect(
                registry.getModel(AuthType.USE_OPENAI, 'model-a')?.name
            ).toBe('Model A First');
            expect(
                registry.getModel(AuthType.USE_OPENAI, 'model-b')?.name
            ).toBe('Model B First');
            expect(
                registry.getModel(AuthType.USE_OPENAI, 'model-c')?.name
            ).toBe('Model C');
        });
    ...
    describe('reloadModels', () => {
        it('should reload models from new config', () => {
            ...
            expect(
                registry.getModel(AuthType.USE_OPENAI, 'gpt-4')
            ).toBeDefined();
            expect(
                registry.getModel(AuthType.USE_OPENAI, 'gpt-3.5')
            ).toBeUndefined();
