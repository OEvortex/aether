...
describe('ModelsConfig', () => {
    ...
    it('should fully rollback state when switchModel fails after applying defaults (authType change)', async () => {
        ...
        expect(modelsConfig.isStrictModelProviderSelection()).toBe(
            baselineStrict
        );
    ...
    it('should fully rollback state when switchModel fails after applying defaults', async () => {
        ...
        expect(modelsConfig.getGenerationConfigSources()).toEqual(
            baselineSources
        );
    });
    ...
    describe('getAllConfiguredModels', () => {
        ...
        it('should support filtering by authTypes and still put aether-oauth first when included', () => {
            ...
            expect(
                openaiOnly.every((m) => m.authType === AuthType.USE_OPENAI)
            ).toBe(true);
    ...
    describe('Runtime Model Snapshot', () => {
        ...
        it('should not capture registry models as runtime', () => {
            ...
            expect(
                modelsConfig.getActiveRuntimeModelSnapshot()
            ).toBeUndefined();
        });
        ...
        it('should switch to runtime model and apply snapshot configuration', async () => {
            ...
            const updatedSnapshotId =
                modelsConfig.getActiveRuntimeModelSnapshotId();
    ...
    describe('reloadModelProvidersConfig', () => {
        ...
        it('should preserve current model selection if still available after reload', async () => {
            ...
            expect(
                availableModels.find((m) => m.id === 'new-model')
            ).toBeDefined();
        });
        ...
        it('should update available models after reload', async () => {
            ...
            expect(initialModels.some((m) => m.id === 'gemini-pro')).toBe(
                false
            );
        ...
        it('should handle reload with empty config', async () => {
            ...
            expect(models.every((m) => m.authType === 'aether-oauth')).toBe(
                true
            );
        });
        ...
        it('should preserve aether-oauth models after reload', () => {
            ...
            const aetherModelsAfterReload = modelsConfig
                .getAllConfiguredModels()
                .filter((m) => m.authType === 'aether-oauth');
            expect(aetherModelsAfterReload.length).toBe(
                initialQwenModels.length
            );
        });
    ...
    describe('max_tokens in modelsConfig', () => {
        ...
        it('should not auto-fill max_tokens for different model families', async () => {
            ...
            await claudeConfig.switchModel(
                AuthType.USE_ANTHROPIC,
                'claude-3-opus'
            );
