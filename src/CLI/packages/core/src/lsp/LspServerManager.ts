...
export class LspServerManager {
    ...
    private attachRestartHandler(name: string, handle: LspServerHandle): void {
        ...
        handle.process.once('exit', (code) => {
            ...
            const maxRestarts =
                handle.config.maxRestarts ?? DEFAULT_LSP_MAX_RESTARTS;
            if (maxRestarts <= 0) {
                handle.status = 'FAILED';
                return;
            }
    ...
    private async createLspConnection(
        ...
        if (config.transport === 'stdio') {
            ...
            const lspConnection =
                await LspConnectionFactory.createStdioConnection(
                    config.command,
                    config.args ?? [],
            ...
            return {
                ...
                exit: () => {
                    if (
                        lspConnection.process &&
                        !lspConnection.process.killed
                    ) {
            ...
            if (!config.socket) {
                throw new Error(
                    'LSP socket transport requires host/port or path'
                );
            }
            ...
            if (config.command) {
                ...
                await new Promise<void>((resolve, reject) => {
                    ...
                    process?.once('error', (error) => {
                        reject(
                            new Error(
                                `Failed to spawn LSP server: ${error.message}`
                            )
                        );
                    });
                });
