...
export class LspResponseNormalizer {
    ...
    normalizeDiagnostic(
        ...
        return {
            ...
            relatedInformation:
                relatedInfo.length > 0 ? relatedInfo : undefined,
            serverName
        };
    }
    ...
    normalizeFileDiagnostics(
        ...
        const itemObj = item as Record<string, unknown>;
        const uri =
            typeof itemObj['uri'] === 'string'
                ? (itemObj['uri'] as string)
                : '';
        if (!uri) {
            return null;
        }
    ...
    normalizeCommand(
        ...
        const title =
            typeof cmdObj['title'] === 'string'
                ? (cmdObj['title'] as string)
                : '';
        const command =
    ...
    normalizeIncomingCall(
        ...
        const itemObj = item as Record<string, unknown>;
        const from = this.normalizeCallHierarchyItem(
            itemObj['from'],
            serverName
        );
        if (!from) {
