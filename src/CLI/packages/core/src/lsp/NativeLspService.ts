...
export class NativeLspService {
    ...
    async hover(
        ...
        for (const [name, handle] of handles) {
            try {
                ...
                const normalized = this.normalizer.normalizeHoverResult(
                    response,
                    name
                );
                if (normalized) {
                ...
                debugLogger.warn(
                    `LSP textDocument/hover failed for ${name}:`,
                    error
                );
            }
        }
            ...
            try {
                ...
                for (const item of response) {
                    ...
                    if (this.normalizer.isDocumentSymbol(itemObj)) {
                        ...
                        const normalized =
                            this.normalizer.normalizeSymbolResult(
                                itemObj,
                                name
                            );
                        if (normalized) {
    ...
    async prepareCallHierarchy(
        ...
        for (const [name, handle] of handles) {
            try {
                ...
                for (const item of candidates) {
                    const normalized =
                        this.normalizer.normalizeCallHierarchyItem(item, name);
                    if (normalized) {
        ...
        for (const [name, handle] of handles) {
            try {
                ...
                for (const call of response) {
                    const normalized = this.normalizer.normalizeIncomingCall(
                        call,
                        name
                    );
                    if (normalized) {
    ...
    async outgoingCalls(
        ...
        for (const [name, handle] of handles) {
            try {
                ...
                for (const call of response) {
                    const normalized = this.normalizer.normalizeOutgoingCall(
                        call,
                        name
                    );
                    if (normalized) {
                ...
                if (response && typeof response === 'object') {
                    ...
                    if (Array.isArray(items)) {
                        for (const item of items) {
                            const normalized =
                                this.normalizer.normalizeDiagnostic(item, name);
                            if (normalized) {
                ...
                if (response && typeof response === 'object') {
                    ...
                    if (Array.isArray(items)) {
                        for (const item of items) {
                            ...
                            const normalized =
                                this.normalizer.normalizeFileDiagnostics(
                                    item,
                                    name
                                );
                            if (
    ...
    async codeActions(
        ...
        for (const [name, handle] of handles) {
            try {
                ...
                for (const item of response) {
                    const normalized = this.normalizer.normalizeCodeAction(
                        item,
                        name
                    );
                    if (normalized) {
    ...
    private async applyTextEdits(
        ...
        if (!this.workspaceContext.isPathWithinWorkspace(filePath)) {
            throw new Error(
                `Refusing to apply edits outside workspace: ${filePath}`
            );
        }
