...
import { DEFAULT_CONTEXT_LENGTH } from '../index.js';
...
export class ChatCompressionService {
    async compress(chat, promptId, force, model, config, hasFailedCompressionAttempt, signal) {
        ...
        const summaryResponse = await config
            .getContentGenerator()
            .generateContent({
            model,
            contents: [
        ...
        if (compressionOutputTokenCount === undefined &&
            ...
            compressionOutputTokenCount = Math.max(0, compressionUsageMetadata.totalTokenCount -
