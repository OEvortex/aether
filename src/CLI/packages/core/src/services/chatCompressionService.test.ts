    ...
    it('should handle history with weird parts', () => {
        const historyWithEmptyParts: Content[] = [
            { role: 'user', parts: [{ text: 'Message 1' }] },
            {
                role: 'model',
                parts: [
                    { fileData: { fileUri: 'derp', mimeType: 'text/plain' } }
                ]
            },
...
describe('ChatCompressionService', () => {
    ...
    it('should compress if over token threshold', async () => {
        ...
        vi.mocked(mockConfig.getContentGeneratorConfig).mockReturnValue({
            ...
            typeof mockConfig.getContentGeneratorConfig
        >);
    ...
    describe('orphaned trailing funcCall handling', () => {
        it('should compress everything when force=true and last message is an orphaned funcCall', async () => {
            ...
            const history: Content[] = [
                {
                    role: 'user',
                    parts: [{ text: 'Fix all TypeScript errors.' }]
                },
                {
                    role: 'model',
            ...
            const mockGenerateContent = vi.fn().mockResolvedValue({
                candidates: [
                    {
                        content: {
                            parts: [{ text: 'Summary of all work done' }]
                        }
                    }
                ],
                usageMetadata: {
