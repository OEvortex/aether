...
export class GitWorktreeService {
    ...
    static getSessionDir(sessionId: string, customBaseDir?: string): string {
        return path.join(
            GitWorktreeService.getBaseDir(customBaseDir),
            sessionId
        );
    }
    ...
    async isGitRepository(): Promise<boolean> {
        try {
            const isRoot = await this.git.checkIsRepo(
    ...
    async setupWorktrees(
        ...
        result.success =
            result.worktrees.length === config.worktreeNames.length;
