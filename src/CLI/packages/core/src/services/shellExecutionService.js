...
export class ShellExecutionService {
    ...
    static cleanup() {
        ...
        // Cleanup PTYs
        for (const [pid, pty] of ShellExecutionService.activePtys) {
            try {
        ...
        strategy.killChildProcesses(ShellExecutionService.activeChildProcesses);
    }
    static {
        process.on('exit', () => {
    ...
    static async execute(commandToExecute, cwd, onOutputEvent, abortSignal, shouldUseNodePty, shellExecutionConfig) {
        if (shouldUseNodePty) {
            ...
            if (ptyInfo) {
                try {
                    return ShellExecutionService.executeWithPty(commandToExecute, cwd, onOutputEvent, abortSignal, shellExecutionConfig, ptyInfo);
                }
                catch (_e) {
        ...
        return ShellExecutionService.childProcessFallback(commandToExecute, cwd, onOutputEvent, abortSignal);
    }
            ...
            const result = new Promise((resolve) => {
                ...
                const handleExit = (code, signal) => {
                    ...
                    const combinedOutput = stdout +
                        (stderr ? (stdout ? separator : '') + stderr : '');
                    const finalStrippedOutput = stripAnsi(combinedOutput).trim();
                    if (isStreamingRawContent) {
                        if (finalStrippedOutput) {
                            onOutputEvent({
                                type: 'data',
                                chunk: finalStrippedOutput
                            });
                        }
                    }
                    else {
                ...
                if (child.pid) {
                    ShellExecutionService.activeChildProcesses.add(child.pid);
                }
                child.on('exit', (code, signal) => {
                    if (child.pid) {
                        ShellExecutionService.activeChildProcesses.delete(child.pid);
                    }
