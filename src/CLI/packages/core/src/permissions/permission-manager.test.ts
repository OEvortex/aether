...
describe('toolMatchesRuleToolName', () => {
    ...
    it('"Read" (read_file) covers grep_search, glob, list_directory', async () => {
        ...
        expect(toolMatchesRuleToolName('read_file', 'list_directory')).toBe(
            true
        );
    });
...
describe('matchesCommandPattern', () => {
    ...
    describe('prefix matching without glob', () => {
        ...
        it('prefix + space', async () => {
            ...
            expect(
                matchesCommandPattern('git commit', 'git commit -m "test"')
            ).toBe(true);
        });
    ...
    describe('wildcard at tail', () => {
        it('matches any arguments', async () => {
            expect(matchesCommandPattern('git *', 'git status')).toBe(true);
            expect(matchesCommandPattern('git *', 'git commit -m "test"')).toBe(
                true
            );
            expect(matchesCommandPattern('npm run *', 'npm run build')).toBe(
                true
            );
        });
    ...
    describe('wildcard at head', () => {
        it('matches any command ending with pattern', async () => {
            expect(matchesCommandPattern('* --version', 'node --version')).toBe(
                true
            );
            expect(matchesCommandPattern('* --version', 'npm --version')).toBe(
                true
            );
            expect(
                matchesCommandPattern('* --help *', 'npm --help install')
            ).toBe(true);
        });

        it('does not match non-matching suffix', async () => {
            expect(matchesCommandPattern('* --version', 'node --help')).toBe(
                false
            );
        });
    });
    ...
    describe('wildcard in middle', () => {
        it('matches middle segments', async () => {
            expect(
                matchesCommandPattern('git * main', 'git checkout main')
            ).toBe(true);
            expect(matchesCommandPattern('git * main', 'git merge main')).toBe(
                true
            );
        });
        ...
        it('does not match different suffix', async () => {
            expect(
                matchesCommandPattern('git * main', 'git checkout dev')
            ).toBe(false);
        });
    });
    ...
    describe('exact command specifier', () => {
        it('Bash(npm run build) matches exact command', async () => {
            expect(
                matchesCommandPattern('npm run build', 'npm run build')
            ).toBe(true);
        });
...
describe('splitCompoundCommand', () => {
    ...
    it('does not split on operators inside single quotes', async () => {
        expect(splitCompoundCommand("echo 'a && b'")).toEqual([
            "echo 'a && b'"
        ]);
    });
...
describe('matchesPathPattern', () => {
    ...
    it('matches dotfiles (e.g. .env)', async () => {
        expect(
            matchesPathPattern('.env', '/project/.env', projectRoot, cwd)
        ).toBe(true);
        expect(
            matchesPathPattern('*.env', '/project/.env', projectRoot, cwd)
        ).toBe(true);
...
describe('matchesDomainPattern', () => {
    ...
    it('matches subdomain', async () => {
        expect(
            matchesDomainPattern('domain:example.com', 'sub.example.com')
        ).toBe(true);
        expect(
    ...
    it('does not match different domain', async () => {
        expect(
            matchesDomainPattern('domain:example.com', 'notexample.com')
        ).toBe(false);
    });
...
describe('matchesRule', () => {
    ...
    it('specifier rule requires a command for shell tools', async () => {
        ...
        expect(matchesRule(rule, 'run_shell_command', 'git status')).toBe(true);
        expect(matchesRule(rule, 'run_shell_command', 'echo hello')).toBe(
            false
        );
    });
    ...
    it('MCP tool exact match', async () => {
        ...
        expect(matchesRule(rule, 'mcp__puppeteer__puppeteer_navigate')).toBe(
            true
        );
        expect(matchesRule(rule, 'mcp__puppeteer__puppeteer_click')).toBe(
            false
        );
    });
...
describe('PermissionManager', () => {
    ...
    describe('isToolEnabled', () => {
        ...
        it('coreTools with specifier: tool-level check strips specifier', async () => {
            ...
            pm = new PermissionManager(
                makeConfig({ coreTools: ['Bash(ls -l)'] })
            );
            pm.initialize();
            expect(await pm.isToolEnabled('run_shell_command')).toBe(true);
        ...
        it('MCP tools bypass coreTools allowlist check', async () => {
            ...
            expect(await pm.isToolEnabled('mcp__puppeteer__navigate')).toBe(
                true
            );
        });
...
describe('getRuleDisplayName', () => {
    ...
    it('returns the canonical name for unknown tools (e.g. MCP)', async () => {
        expect(getRuleDisplayName('mcp__server__tool')).toBe(
            'mcp__server__tool'
        );
    });
});
...
describe('buildHumanReadableRuleLabel', () => {
    ...
    it('converts bare WebSearch rule to "search the web"', () => {
        expect(buildHumanReadableRuleLabel(['WebSearch'])).toBe(
            'search the web'
        );
    });
