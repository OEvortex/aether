...
export class Config {
    ...
    constructor(params: ConfigParameters) {
        ...
        this.telemetrySettings = {
            ...
            otlpEndpoint:
                params.telemetry?.otlpEndpoint ?? DEFAULT_OTLP_ENDPOINT,
        ...
        this.fileFiltering = {
            ...
            respectAetherIgnore:
                params.fileFiltering?.respectAetherIgnore ?? true,
        ...
        this.modelsConfig = new ModelsConfig({
            initialAuthType:
                params.authType ?? params.generationConfig?.authType,
    ...
    async initialize(options?: ConfigInitializeOptions): Promise<void> {
        ...
        if (!this.disableAllHooks) {
            ...
            this.debugLogger.debug(
                'MessageBus initialized with hook subscription'
            );
        } else {
            this.debugLogger.debug(
                'Hook system disabled, skipping initialization'
            );
        }
    ...
    updateCredentials(
        ...
        this.modelsConfig.updateCredentials(
            credentials,
    ...
    getModel(): string {
        return (
            this.contentGeneratorConfig?.model || this.modelsConfig.getModel()
        );
    }
    ...
    setSessionSubagents(subagents: SubagentConfig[]): void {
        if (this.initialized) {
            throw new Error(
                'Cannot modify sessionSubagents after initialization'
            );
        }
