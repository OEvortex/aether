...
describe('WorkspaceContext with real filesystem', () => {
    ...
    describe('path validation', () => {
        it('should accept paths within workspace directories', () => {
            ...
            expect(workspaceContext.isPathWithinWorkspace(validPath1)).toBe(
                true
            );
            expect(workspaceContext.isPathWithinWorkspace(validPath2)).toBe(
                true
            );
        });
        ...
        it('should reject paths outside workspace', () => {
            ...
            expect(workspaceContext.isPathWithinWorkspace(invalidPath)).toBe(
                false
            );
        });
        ...
        it('should handle nested directories correctly', () => {
            ...
            expect(workspaceContext.isPathWithinWorkspace(nestedPath)).toBe(
                true
            );
        });
        ...
        it('should handle edge cases (root, parent references)', () => {
            ...
            expect(workspaceContext.isPathWithinWorkspace(rootPath)).toBe(
                false
            );
            expect(workspaceContext.isPathWithinWorkspace(parentPath)).toBe(
                false
            );
        });
        ...
        it('should handle non-existent paths correctly', () => {
            ...
            expect(
                workspaceContext.isPathWithinWorkspace(nonExistentPath)
            ).toBe(true);
        });
        ...
        describe('with symbolic link', () => {
            describe('in the workspace', () => {
                ...
                it('should accept dir paths', () => {
                    ...
                    expect(
                        workspaceContext.isPathWithinWorkspace(symlinkDir)
                    ).toBe(true);
                });
            ...
            it('should reject symbolic file links outside the workspace', () => {
                ...
                expect(
                    workspaceContext.isPathWithinWorkspace(symlinkFile)
                ).toBe(false);
            });
            ...
            it('should handle circular symlinks gracefully', () => {
                ...
                expect(workspaceContext.isPathWithinWorkspace(linkA)).toBe(
                    false
                );
                expect(workspaceContext.isPathWithinWorkspace(linkB)).toBe(
                    false
                );
            });
        });
...
describe('WorkspaceContext with optional directories', () => {
    ...
    beforeEach(() => {
        tempDir = fs.realpathSync(
            fs.mkdtempSync(
                path.join(os.tmpdir(), 'workspace-context-optional-')
            )
        );
        cwd = path.join(tempDir, 'project');
