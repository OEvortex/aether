...
describe('secure-browser-launcher', () => {
    ...
    describe('URL validation', () => {
        ...
        it('should reject non-HTTP(S) protocols', async () => {
            await expect(
                openBrowserSecurely('file:///etc/passwd')
            ).rejects.toThrow('Unsafe protocol');
            await expect(
                openBrowserSecurely('javascript:alert(1)')
            ).rejects.toThrow('Unsafe protocol');
            await expect(
                openBrowserSecurely('ftp://example.com')
            ).rejects.toThrow('Unsafe protocol');
        });
        ...
        it('should reject invalid URLs', async () => {
            ...
            await expect(openBrowserSecurely('')).rejects.toThrow(
                'Invalid URL'
            );
        });
