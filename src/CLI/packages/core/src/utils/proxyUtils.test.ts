...
describe('normalizeProxyUrl', () => {
    ...
    it('should add http:// prefix to proxy URL with port only', () => {
        expect(normalizeProxyUrl('localhost:8080')).toBe(
            'http://localhost:8080'
        );
    });
    ...
    it('should handle IPv6 addresses with http:// prefix', () => {
        expect(normalizeProxyUrl('http://[::1]:8080')).toBe(
            'http://[::1]:8080'
        );
    });
    ...
    it('should throw error for socks:// proxy URL', () => {
        expect(() =>
            normalizeProxyUrl('socks://proxy.example.com:1080')
        ).toThrow('SOCKS proxy is not supported');
    });

    it('should throw error for socks4:// proxy URL', () => {
        expect(() =>
            normalizeProxyUrl('socks4://proxy.example.com:1080')
        ).toThrow('SOCKS proxy is not supported');
    });
