...
const QWEN35_1M_MAX_INPUT_TOKENS = QWEN35_1M_TOTAL_TOKENS - QWEN35_1M_MAX_OUTPUT_TOKENS;
...
const QWEN36_1M_MAX_INPUT_TOKENS = QWEN36_1M_TOTAL_TOKENS - QWEN36_1M_MAX_OUTPUT_TOKENS;
...
const MIMOV2_PRO_MAX_INPUT_TOKENS = MIMOV2_PRO_TOTAL_TOKENS - MIMOV2_PRO_MAX_OUTPUT_TOKENS; // 983040
// MiMo v2 Omni: 256K total context, 32K output / 224K input
const MIMOV2_OMNI_MAX_INPUT_TOKENS = 256 * TOKENS_PER_KIBI - 32 * TOKENS_PER_KIBI; // 229376
...
export function isDevstralModel(modelId) {
...
export function isGemini2Model(modelId) {
...
export function isGlm45Model(modelId) {
...
export function isGlmModel(modelId) {
...
export function isGpt41Model(modelId) {
    // Examples: gpt-4-1, gpt-4-1-mini, gpt-4-1-nano
    return /gpt-4-1/i.test(modelId);
}
export function isGpt4oModel(modelId) {
...
export function isQwen36OneMillionContextModel(modelId) {
...
export function isKimiK25Model(modelId) {
    return /kimi[-_/]?k2(?:\.|-)5/i.test(modelId);
}
export function isKimiModel(modelId) {
...
export function isMinimaxModel(modelId) {
...
export function resolveGlobalTokenLimits(modelId, contextLength, options) {
    ...
    const minReservedInputTokens = typeof options.minReservedInputTokens === 'number' &&
        options.minReservedInputTokens > 0
        ? options.minReservedInputTokens
        : DEFAULT_MIN_RESERVED_INPUT_TOKENS;
    const safeContextLength = typeof contextLength === 'number' &&
        contextLength > minReservedInputTokens
        ? contextLength
        : options.defaultContextLength;
    let maxOutput = getDefaultMaxOutputTokensForContext(safeContextLength, options.defaultMaxOutputTokens);
    maxOutput = Math.floor(Math.max(1, Math.min(maxOutput, safeContextLength - minReservedInputTokens)));
    return {
        maxInputTokens: Math.max(1, safeContextLength - maxOutput),
        maxOutputTokens: maxOutput
    };
}
export function resolveAdvertisedTokenLimits(modelId, advertisedContextLength, options) {
    const resolvedLimits = resolveGlobalTokenLimits(modelId, advertisedContextLength ?? options.defaultContextLength, options);
    const advertisedMaxOutputTokens = typeof options.advertisedMaxOutputTokens === 'number' &&
    ...
    const totalContextTokens = resolvedLimits.maxInputTokens + resolvedLimits.maxOutputTokens;
    const boundedMaxOutputTokens = Math.min(advertisedMaxOutputTokens, Math.max(1, totalContextTokens - 1));
    return {
        maxInputTokens: Math.max(1, totalContextTokens - boundedMaxOutputTokens),
        maxOutputTokens: boundedMaxOutputTokens
    };
}
export function resolveGlobalCapabilities(modelId, options) {
...
//# sourceMappingURL=globalContextLengthManager.js.map