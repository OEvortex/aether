...
export class SchemaValidator {
    ...
    static validate(schema, data) {
        ...
        try {
            ...
            debugLogger.warn(`Failed to compile schema (${schema?.$schema ??
                '<no $schema>'}): ${error instanceof Error ? error.message : String(error)}. ` +
        ...
        if (!valid && validate.errors) {
            ...
            valid = validate(data);
            if (!valid && validate.errors) {
                return validator.errorsText(validate.errors, {
                    dataVar: 'params'
                });
            }
        }
