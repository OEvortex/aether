...
describe('memoryImportProcessor', () => {
    ...
    describe('processImports', () => {
        it('should process basic md file imports', async () => {
            ...
            expect(
                comments.some((c) =>
                    c.includes('End of import from: ./test.md')
                )
            ).toBe(true);
        ...
        it('should import non-md files just like md files', async () => {
            ...
            expect(
                comments.some((c) =>
                    c.includes('Imported from: ./instructions.txt')
                )
            ).toBe(true);
            expect(
                comments.some((c) =>
            ...
            expect(
                headers.some(
                    (h) => (h as { text: string }).text === 'Instructions'
                )
            ).toBe(true);
        ...
        it('should ignore imports inside code blocks', async () => {
            ...
            const comments = findMarkdownComments(result.content);
            expect(
                comments.some((c) => c.includes('should-not-import.md'))
            ).toBe(false);
        });
        ...
        it('should handle nested tokens and non-unique content correctly', async () => {
            ...
            expect(result.content).toContain(
                '`inline code @./should-not-import.md`'
            );
        ...
        it('should build import tree structure', async () => {
            ...
            expect(importComments.some((c) => c.includes('./nested.md'))).toBe(
                true
            );
            expect(importComments.some((c) => c.includes('./simple.md'))).toBe(
                true
            );
            expect(importComments.some((c) => c.includes('./inner.md'))).toBe(
                true
            );
            ...
            const expectedNestedPath = testPath(
                projectRoot,
                'src',
                'nested.md'
            );
            ...
            const expectedSimplePath = testPath(
                projectRoot,
                'src',
                'simple.md'
            );
    ...
    describe('validateImportPath', () => {
        it('should reject URLs', () => {
            ...
            expect(
                validateImportPath('file:///path/to/file.md', basePath, [
                    allowedPath
                ])
            ).toBe(false);
        });
        ...
        it('should allow paths within allowed directories', () => {
            ...
            expect(validateImportPath(relativePath, basePath, [basePath])).toBe(
                true
            );
            ...
            if (parentPath !== basePath) {
                ...
                expect(
                    validateImportPath(parentRelativePath, basePath, [
                        parentPath
                    ])
                ).toBe(true);

                path.resolve(basePath, 'sub');
                const resultSub = validateImportPath('sub', basePath, [
                    basePath
                ]);
            ...
            const allowedFilePath = path.join(
                allowedPath,
                allowedSubPath,
                'file.md'
            );
            expect(
                validateImportPath(allowedFilePath, basePath, [allowedPath])
            ).toBe(true);
        });
        ...
        it('should reject paths outside allowed directories', () => {
            ...
            expect(
                validateImportPath(forbiddenPath, basePath, [allowedPath])
            ).toBe(false);
        ...
        it('should handle relative paths correctly', () => {
            ...
            expect(
                validateImportPath(relativeToParent, basePath, [basePath])
            ).toBe(false);

            // Parent directory access when parent is in allowed paths
            expect(
                validateImportPath(relativeToParent, basePath, [
                    basePath,
                    parentPath
                ])
            ).toBe(true);
            ...
            expect(validateImportPath(nestedPath, basePath, [basePath])).toBe(
                true
            );
        });
        ...
        it('should handle absolute paths correctly', () => {
            ...
            const allowedNestedPath = path.join(
                allowedPath,
                'nested',
                'file.md'
            );
            ...
            expect(validateImportPath(dotPath, basePath, [allowedPath])).toBe(
                true
            );
        });
    });
