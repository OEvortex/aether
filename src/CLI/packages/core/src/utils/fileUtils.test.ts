...
describe('fileUtils', () => {
    ...
    describe('isWithinRoot', () => {
        ...
        it('should return true for paths directly within the root', () => {
            ...
            expect(
                isWithinRoot(path.join(root, 'subdir', 'file.txt'), root)
            ).toBe(true);
        });
        ...
        it('should return false for paths outside the root', () => {
            ...
            expect(
                isWithinRoot(path.resolve('/unrelated', 'file.txt'), root)
            ).toBe(false);
        });
        ...
        it('should handle paths with trailing slashes correctly', () => {
            expect(
                isWithinRoot(path.join(root, 'file.txt') + path.sep, root)
            ).toBe(true);
        ...
        describe('readFileWithEncoding', () => {
            ...
            it('should read GBK-encoded file with Chinese characters correctly', async () => {
                ...
                const gbkBuffer = Buffer.from([
                    0xc4, 0xe3, 0xba, 0xc3, 0xca, 0xc0, 0xbd, 0xe7, 0xd5, 0xe2,
                    0xca, 0xc7, 0xd6, 0xd0, 0xce, 0xc4, 0xc4, 0xda, 0xc8, 0xdd,
                    0xd3, 0xc3, 0xd3, 0xda, 0xb2, 0xe2, 0xca, 0xd4, 0xb1, 0xe0,
                    0xc2, 0xeb, 0xbc, 0xec, 0xb2, 0xe2
                ]);
            ...
            it('should read GBK-encoded file with mixed ASCII and Chinese correctly', async () => {
                ...
                const gbkBuffer = Buffer.from([
                    0x2f, 0x2f, 0x20, 0xd5, 0xe2, 0xca, 0xc7, 0xd7, 0xa2, 0xca,
                    0xcd, 0xc4, 0xda, 0xc8, 0xdd, 0xd3, 0xc3, 0xd3, 0xda, 0xb2,
                    0xe2, 0xca, 0xd4, 0x0a, 0x68, 0x65, 0x6c, 0x6c, 0x6f, 0xc4,
                    0xe3, 0xba, 0xc3, 0xca, 0xc0, 0xbd, 0xe7, 0xb2, 0xe2, 0xca,
                    0xd4, 0xd6, 0xd0, 0xce, 0xc4, 0xb1, 0xe0, 0xc2, 0xeb, 0xbc,
                    0xec, 0xb2, 0xe2, 0x0a, 0xba, 0xaf, 0xca, 0xfd, 0xb7, 0xb5,
                    0xbb, 0xd8, 0xd6, 0xb5, 0xd5, 0xfd, 0xc8, 0xb7
                ]);
        ...
        describe('isBinaryFile with BOM awareness', () => {
            ...
            it('should treat file with null bytes (no BOM) as binary', async () => {
                const content = Buffer.from([
                    0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x00, 0x77, 0x6f, 0x72, 0x6c,
                    0x64
                ]);
    ...
    describe('processSingleFileContent', () => {
        ...
        it('should process an image file', async () => {
            ...
            expect(
                (result.llmContent as { inlineData: { mimeType: string } })
                    .inlineData.mimeType
            ).toBe('image/png');
            expect(
                (result.llmContent as { inlineData: { data: string } })
                    .inlineData.data
  ...
  `;
        ...
        it('should paginate text files correctly (offset and limit)', async () => {
            ...
            expect(result.returnDisplay).toBe(
                'Read lines 6-10 of 20 from test.txt'
            );
        ...
        it('should identify truncation when reading the end of a file', async () => {
            ...
            expect(result.returnDisplay).toBe(
                'Read lines 11-20 of 20 from test.txt'
            );
