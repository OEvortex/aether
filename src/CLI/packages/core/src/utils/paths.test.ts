...
describe('escapePath', () => {
    ...
    it('should escape tabs', () => {
        expect(escapePath('file\twith\ttabs.txt')).toBe(
            'file\\\twith\\\ttabs.txt'
        );
    });
    ...
    it('should handle escaped backslashes correctly', () => {
        ...
        expect(escapePath('path\\\\\\\\ file.txt')).toBe(
            'path\\\\\\\\\\ file.txt'
        );
    });
    ...
    it('should handle paths without special characters', () => {
        ...
        expect(escapePath('path/to/normalfile.txt')).toBe(
    ...
    it('should handle complex real-world examples', () => {
        expect(
            escapePath('My Documents/Project (2024)/file [backup].txt')
        ).toBe('My\\ Documents/Project\\ \\(2024\\)/file\\ \\[backup\\].txt');
...
describe('isSubpath on Windows', () => {
    ...
    it('should handle trailing slashes on Windows', () => {
        expect(
            isSubpath('C:\\Users\\Test\\', 'C:\\Users\\Test\\file.txt')
        ).toBe(true);
    });
...
describe('validatePath', () => {
    ...
    it('validates paths in allowed directories', () => {
        const extraDir = fs.mkdtempSync(
            path.join(os.tmpdir(), 'validate-extra-')
        );
        try {
...
describe('resolveAndValidatePath', () => {
    ...
    it('allows absolute paths that are permitted by the workspace context', () => {
        ...
        try {
            ...
            expect(resolveAndValidatePath(configWithExtra, extraDir)).toBe(
                extraDir
            );
        } finally {
            fs.rmSync(extraDir, { recursive: true, force: true });
    ...
    it('throws when the path points to a file (default behavior)', () => {
        ...
        try {
            expect(() =>
                resolveAndValidatePath(config, 'file.txt')
            ).toThrowError(`Path is not a directory: ${filePath}`);
        } finally {
            fs.rmSync(filePath);
        }
    });
...
describe('shortenPath', () => {
    ...
    it('preserves the root directory in shortened paths', () => {
        const result = shortenPath(
            `${sep}a${sep}b${sep}c${sep}d${sep}e.txt`,
            15
        );
    ...
    it('correctly calculates length including ellipsis', () => {
        ...
        if (result.includes('...')) {
            ...
            expect(parts[0].length + 3 + parts[1].length).toBeLessThanOrEqual(
                maxLen
            );
        }
    });
    ...
    it('example from documentation: /path/to/a/very/long/file.txt', () => {
        ...
        expect(result).toMatch(
            new RegExp(
                `^${sepForRegex}path${sepForRegex}\\.\\.\\..+file\\.txt$`
            )
        );
        expect(result.length).toBeLessThanOrEqual(35);
    });
