...
import { normalizeContent, safeLiteralReplace } from './textUtils.js';
...
describe('safeLiteralReplace', () => {
    ...
    it("does not interpret replacement patterns like $&, $', $` and $1", () => {
        ...
        expect(safeLiteralReplace('test', 'test', '$1$2value')).toBe(
            '$1$2value'
        );
    });
...
describe('normalizeContent', () => {
    ...
    it('handles strings without newlines or BOM', () => {
        expect(normalizeContent('Just a single line')).toBe(
            'Just a single line'
        );
    });
});
