...
describe('GitIgnoreParser', () => {
    ...
    beforeEach(async () => {
        projectRoot = await fs.mkdtemp(
            path.join(os.tmpdir(), 'gitignore-test-')
        );
...
`;
            ...
            expect(
                parser.isIgnored(path.join('node_modules', 'some-lib'))
            ).toBe(true);
...
`;
        ...
        it('should ignore files with path-specific patterns', () => {
            ...
            expect(parser.isIgnored(path.join('other', 'temp.tmp'))).toBe(
                false
            );
        });

        it('should handle negation patterns', () => {
            expect(parser.isIgnored(path.join('src', 'important.tmp'))).toBe(
                false
            );
        });
        ...
        it('should handle relative paths correctly', () => {
            expect(
                parser.isIgnored(path.join('node_modules', 'some-package'))
            ).toBe(true);
            expect(
        ...
        it('should normalize path separators on Windows', () => {
            expect(parser.isIgnored(path.join('node_modules', 'package'))).toBe(
                true
            );
            expect(parser.isIgnored(path.join('src', 'temp.tmp'))).toBe(true);
        });
        ...
        it('should handle backslash-prefixed files without crashing', () => {
            expect(() =>
                parser.isIgnored('\\backslash-file-test.txt')
            ).not.toThrow();
    ...
    describe('precedence rules', () => {
        ...
        it('should prioritize .gitignore over .git/info/exclude', async () => {
            ...
            expect(parser.isIgnored(path.join('subdir', 'some.log'))).toBe(
                true
            );
