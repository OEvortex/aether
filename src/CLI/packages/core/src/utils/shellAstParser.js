...
// Constants
// ---------------------------------------------------------------------------
function resolveModuleFilePath(moduleFilePath) {
    try {
...
function resolveWasmPath(filename) {
    ...
    if (rawPath.includes(path.join('src', 'utils'))) {
        ...
        return path.join(path.dirname(rawPath), ...Array(levelsUp).fill('..'), 'vendor', 'tree-sitter', filename);
    }
    ...
    return path.join(candidateDirs[0] ?? path.dirname(rawPath), 'vendor', 'tree-sitter', filename);
}
/**
 ...
 */
function getWasmCandidateDirs(rawModulePath) {
...
function resolveWasmPathForModule(filename, moduleFilePath, resolvePath = resolveModuleFilePath) {
    ...
    const levelsUp = !inSrcUtils
        ...
        : resolvedModuleFilePath.endsWith('.ts')
            ? 2
            : 3;
    return path.join(moduleDir, ...Array(levelsUp).fill('..'), 'vendor', 'tree-sitter', filename);
}
/**
...
export async function initParser() {
    ...
    initPromise = (async () => {
        ...
        bashLanguage = await Parser.Language.load(resolveWasmPath('tree-sitter-bash.wasm'));
 ...
 * @internal
 */
export function _resolveWasmPathForTesting(filename, moduleFilePath, resolvePath) {
