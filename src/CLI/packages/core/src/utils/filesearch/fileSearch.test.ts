...
import { cleanupTmpDir, createTmpDir } from '../../aetherTestUtils.js';
...
describe('FileSearch', () => {
    ...
    it('should throw AbortError when filter is aborted', async () => {
        ...
        const dummyPaths = Array.from(
            { length: 5000 },
            (_, i) => `file${i}.js`
        ); // Large array to ensure yielding
    ...
    it('should respect maxResults even when the cache returns an exact match', async () => {
        ...
        const limitedResults = await fileSearch.search('*.js', {
            maxResults: 2
        });
