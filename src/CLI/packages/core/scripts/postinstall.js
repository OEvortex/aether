...
function setupRipgrepBinaries() {
    try {
        ...
        if (!fs.existsSync(rgBinary)) {
            console.log(
                `ℹ Ripgrep binary not found at ${rgBinary}, skipping setup`
            );
            return;
        }
        ...
        try {
            ...
            if (platform === 'darwin') {
                try {
                    ...
                    console.log(
                        `✓ Removed quarantine attribute from ${rgBinary}`
                    );
                } catch {
                    // Quarantine attribute might not exist, which is fine
                    console.log(
                        'ℹ Quarantine attribute not present or already removed'
                    );
                }
            }
        } catch (error) {
            ...
            console.log(
                '  This is not critical - ripgrep may still work correctly'
            );
        }
    } catch (error) {
        console.log(
        ...
        console.log(
            '  Continuing anyway - this should not affect functionality'
        );
    }
}
