    ...
    describe('getSettingsSchema', () => {
        it('should contain all expected top-level settings', () => {
            ...
            expectedSettings.forEach((setting) => {
                expect(
                    getSettingsSchema()[setting as keyof Settings]
                ).toBeDefined();
            });
        });

        it('should have correct structure for each setting', () => {
            Object.entries(getSettingsSchema()).forEach(
                ([_key, definition]) => {
        ...
        it('should have checkpointing nested properties', () => {
            expect(
                getSettingsSchema().general?.properties?.checkpointing
                    .properties?.enabled
            ).toBeDefined();
            expect(
                getSettingsSchema().general?.properties?.checkpointing
            ...
            Object.values(getSettingsSchema()).forEach((definition) => {
                ...
                if (defWithProps.properties) {
                    Object.values(defWithProps.properties).forEach(
                        (nestedDef: unknown) => {
                            const nestedDefTyped = nestedDef as {
                                category?: string;
                            };
        ...
        it('should have showInDialog property configured', () => {
            ...
            expect(
                getSettingsSchema().general.properties.vimMode.showInDialog
            ).toBe(true);
            expect(
                getSettingsSchema().ide.properties.enabled.showInDialog
            ).toBe(true);
            expect(
                getSettingsSchema().general.properties.enableAutoUpdate
                    .showInDialog
            ).toBe(true);
            expect(
            ...
            expect(
                getSettingsSchema().ui.properties.hideTips.showInDialog
            ).toBe(true);
            expect(
            ...
            expect(
                getSettingsSchema().security.properties.auth.showInDialog
            ).toBe(false);
            ...
            expect(getSettingsSchema().ui.properties.theme.showInDialog).toBe(
                true
            );
            expect(
                getSettingsSchema().ui.properties.customThemes.showInDialog
            ).toBe(false); // Managed via theme editor
            expect(
                getSettingsSchema().general.properties.checkpointing
                    .showInDialog
            ).toBe(false); // Experimental feature
            expect(
                getSettingsSchema().ui.properties.accessibility.showInDialog
            ).toBe(false);
            expect(
                getSettingsSchema().context.properties.fileFiltering
                    .showInDialog
            ).toBe(false);
            expect(
                getSettingsSchema().general.properties.preferredEditor
                    .showInDialog
            ).toBe(true);
            expect(
        ...
        it('should have loadFromIncludeDirectories setting in schema', () => {
            ...
            expect(
                getSettingsSchema().context?.properties
                    .loadFromIncludeDirectories.type
            ).toBe('boolean');
            expect(
                getSettingsSchema().context?.properties
                    .loadFromIncludeDirectories.category
            ).toBe('Context');
            expect(
                getSettingsSchema().context?.properties
                    .loadFromIncludeDirectories.default
            ).toBe(false);
        });

        it('should have folderTrustFeature setting in schema', () => {
            expect(
                getSettingsSchema().security.properties.folderTrust.properties
                    .enabled
            ).toBeDefined();
            expect(
                getSettingsSchema().security.properties.folderTrust.properties
