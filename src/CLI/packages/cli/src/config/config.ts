...
export async function parseArguments(): Promise<CliArgs> {
    ...
    const yargsInstance = yargs(rawArgv)
        ...
        .option('proxy', {
            type: 'string',
            description:
                'Proxy for Aether Code, like schema://user:password@host:port'
        })
        .deprecateOption(
            'proxy',
        ...
        .command(
            ...
            (yargsInstance: Argv) =>
                yargsInstance
                    ...
                    .fail(
                        (msg: string, err: Error | undefined, yargs: Argv) => {
                            writeStderrLine(
    ...
    const queryArg = (result as { query?: string | string[] | undefined })
        .query;
...
export async function loadCliConfig(
        ...
        if (
            mergedAllow.some((rule) => {
                ...
                const ruleName =
                    openParen === -1
                        ? rule.trim()
                        : rule.substring(0, openParen).trim();
    ...
    const runtimeModelProvidersConfig =
        buildModelProvidersConfigFromProviderRegistry(
    ...
    const config = new Config({
        ...
        onPersistPermissionRule: async (scope, ruleType, rule) => {
            ...
            if (!currentRules.includes(rule)) {
                currentSettings.setValue(settingScope, key, [
                    ...currentRules,
                    rule
                ]);
            }
        ...
        truncateToolOutputThreshold:
            settings.tools?.truncateToolOutputThreshold,
        ...
        agents: settings.agents
            ? {
                  ...
                  arena: settings.agents.arena
                      ? {
                            worktreeBaseDir:
                                settings.agents.arena.worktreeBaseDir,
                            preserveArtifacts:
