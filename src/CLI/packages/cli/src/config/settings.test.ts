...
import { AETHER_DIR, FatalConfigError } from '@aetherai/aether-core';
...
describe('Settings Loading and Merging', () => {
    ...
    describe('loadSettings', () => {
        ...
        it('should rewrite allowedTools to tools.allowed during migration', () => {
            ...
            expect(
                (settings.merged as TestSettings)['allowedTools']
            ).toBeUndefined();
        });
        ...
        it('should add version field to V2 settings without version and write to disk', () => {
            ...
            if (!writeCall) {
                throw new Error(
                    'Expected temp write call for version normalization'
                );
            }
            const writtenContent = JSON.parse(writeCall[1] as string);
        ...
        it('should bump version to 3 even when V2 settings already have V3-compatible content', () => {
            ...
            if (!writeCall) {
                throw new Error(
                    'Expected temp write call for V2->V3 version bump'
                );
            }
            const writtenContent = JSON.parse(writeCall[1] as string);
        ...
        it('should handle contextFileName correctly when only in workspace settings', () => {
            ...
            const settings = loadSettings(MOCK_WORKSPACE_DIR);
            expect(settings.merged.context?.fileName).toBe(
                'PROJECT_SPECIFIC.md'
            );
        });
        ...
        it('should handle excludedProjectEnvVars correctly when only in user settings', () => {
            ...
            const userSettingsContent = {
                general: {},
                advanced: {
                    excludedEnvVars: ['DEBUG', 'NODE_ENV', 'CUSTOM_VAR']
                }
            };
            (fs.readFileSync as Mock).mockImplementation(
                (p: fs.PathOrFileDescriptor) => {
        ...
        it('should handle excludedProjectEnvVars correctly when only in workspace settings', () => {
            ...
            const workspaceSettingsContent = {
                general: {},
                advanced: {
                    excludedEnvVars: ['WORKSPACE_DEBUG', 'WORKSPACE_VAR']
                }
            };
            (fs.readFileSync as Mock).mockImplementation(
                (p: fs.PathOrFileDescriptor) => {
        ...
        it('should merge excludedProjectEnvVars with workspace taking precedence over user', () => {
            ...
            expect(
                settings.workspace.settings.advanced?.excludedEnvVars
        ...
        it('should handle JSON parsing errors gracefully', () => {
            ...
            try {
                loadSettings(MOCK_WORKSPACE_DIR);
                throw new Error(
                    'loadSettings should have thrown a FatalConfigError'
                );
            } catch (e) {
        ...
        it('should resolve environment variables in workspace settings', () => {
            ...
            const settings = loadSettings(MOCK_WORKSPACE_DIR);
            expect(
                (settings.workspace.settings as TestSettings)['endpoint']
        ...
        it('should correctly resolve and merge env variables from different scopes', () => {
            ...
            expect(
                (settings.system.settings as TestSettings)['configValue']
            ).toBe('final_value');
            expect(
                (settings.system.settings as TestSettings)['systemOnly']
            ).toBe('system_value');
            expect(
                (settings.user.settings as TestSettings)['configValue']
            ).toBe('final_value');
            expect((settings.user.settings as TestSettings)['userOnly']).toBe(
                'user_value'
            );
            expect(
                (settings.workspace.settings as TestSettings)['configValue']
            ).toBe('final_value');
            expect(
            ...
            expect((settings.merged as TestSettings)['userOnly']).toBe(
                'user_value'
            );
        ...
        it('should correctly merge dnsResolutionOrder with workspace taking precedence', () => {
            ...
            const settings = loadSettings(MOCK_WORKSPACE_DIR);
            expect(settings.merged.advanced?.dnsResolutionOrder).toBe(
                'verbatim'
            );
        });
        ...
        it('should correctly pass through null, boolean, and number types, and handle undefined properties', () => {
            ...
            expect(
                (settings.user.settings as TestSettings)['nullVal']
            ).toBeNull();
            expect((settings.user.settings as TestSettings)['trueVal']).toBe(
                true
            );
            expect((settings.user.settings as TestSettings)['falseVal']).toBe(
                false
            );
        ...
        it('should resolve multiple concatenated environment variables in a single string value', () => {
            ...
            const settings = loadSettings(MOCK_WORKSPACE_DIR);
            expect(
                (settings.user.settings as TestSettings)['serverAddress']
        ...
        describe('when aether_cli_SYSTEM_SETTINGS_PATH is set', () => {
            const MOCK_ENV_SYSTEM_SETTINGS_PATH =
            ...
            it('should load system settings from the path specified in the environment variable', () => {
                ...
                expect(settings.system.path).toBe(
                    MOCK_ENV_SYSTEM_SETTINGS_PATH
                );
                expect(settings.system.settings).toEqual({
                    ...systemSettingsContent,
    ...
    describe('excludedProjectEnvVars integration', () => {
        ...
        it('should exclude DEBUG and DEBUG_MODE from project .env files by default', () => {
            ...
            try {
                ...
                (
                    loadSettings as unknown as { findEnvFile: () => string }
                ).findEnvFile = originalFindEnvFile;
                (fs.readFileSync as Mock).mockImplementation(
    ...
    describe('loadEnvironment', () => {
        ...
        describe('settings.env field', () => {
            ...
            it('should load environment variables from settings.env as fallback', () => {
                ...
                expect(process.env['ENV_FROM_SETTINGS']).toEqual(
                    'settings_value'
                );
            });
            ...
            it('should allow .env file to override settings.env values', () => {
                ...
                (mockFsExistsSync as Mock).mockImplementation(
                    (p: fs.PathLike) =>
                        [USER_SETTINGS_PATH, geminiEnvPath].includes(
                            p.toString()
                        )
                );
                (fs.readFileSync as Mock).mockImplementation(
            ...
            it('should load settings.env from both user and workspace settings', () => {
                ...
                expect(process.env['WORKSPACE_ENV_VAR']).toEqual(
                    'workspace_value'
                );
            });
    ...
    describe('needsMigration', () => {
        ...
        describe('edge case: partially migrated settings', () => {
            ...
            it('should return false for partially migrated settings WITH version field', () => {
                ...
                expect(needsMigration(partiallyMigratedWithVersion)).toBe(
                    false
                );
            });
        });
