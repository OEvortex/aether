...
export function getSettingsWarnings(loadedSettings: LoadedSettings): string[] {
    ...
    for (const scope of [SettingScope.User, SettingScope.Workspace]) {
        ...
        const settingsObject =
            settingsFile.originalSettings as unknown as Record<string, unknown>;
...
export function loadEnvironment(settings: Settings): void {
    ...
    if (envFilePath) {
        try {
            ...
            const excludedVars =
                settings?.advanced?.excludedEnvVars ||
            ...
            if (fs.existsSync(filePath)) {
                ...
                const persistSettingsObject = (warningPrefix: string) => {
                    try {
                        ...
                        debugLogger.error(
                            `${warningPrefix}: ${getErrorMessage(e)}`
                        );
                    }
                };
                ...
                if (needsMigration(settingsObject)) {
                    ...
                    if (migrationResult.executedMigrations.length > 0) {
                        ...
                        persistSettingsObject(
                            'Error migrating settings file on disk'
                        );
                        ...
                        persistSettingsObject(
                            'Error normalizing settings version on disk'
                        );
                    }
                } else if (
                    !hasVersionKey ||
