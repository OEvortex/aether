...
export class SystemController extends BaseController {
    ...
    protected async handleRequestPayload(
        ...
        switch (payload.subtype) {
            ...
            default:
                throw new Error(
                    `Unsupported request subtype in SystemController`
                );
        }
    }
    ...
    private async handleInitialize(
        ...
        if (
            ...
            for (const [key, wireConfig] of Object.entries(
                payload.sdkMcpServers
            )) {
                const name =
                    typeof wireConfig?.name === 'string' &&
                    wireConfig.name.trim().length
                        ? wireConfig.name
                        : key;
     ...
     */
    private async loadSlashCommandNames(
        signal: AbortSignal
    ): Promise<string[]> {
        if (signal.aborted) {
            return [];
