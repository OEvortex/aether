...
import { InputFormat, ToolConfirmationOutcome } from '@aetherai/aether-core';
import type {
...
export class PermissionController extends BaseController {
    ...
    protected async handleRequestPayload(
        ...
        switch (payload.subtype) {
            ...
            default:
                throw new Error(
                    `Unsupported request subtype in PermissionController`
                );
        }
    }
        ...
        return (toolCalls: unknown[]) => {
            for (const call of toolCalls) {
                if (
                    ...
                    if (
                        typeof awaiting.confirmationDetails?.onConfirm ===
                            'function' &&
                        !this.pendingOutgoingRequests.has(
                            awaiting.request.callId
                        )
                    ) {
                        this.pendingOutgoingRequests.add(
                            awaiting.request.callId
                        );
    ...
    private async handleOutgoingPermissionRequest(
        ...
        try {
            ...
            const payload = (response.response || {}) as Record<
                string,
                unknown
            >;
