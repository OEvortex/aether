...
import type { Config, ConfigInitializeOptions } from '@aetherai/aether-core';
...
class Session {
    ...
    private async initializeSdkMode(request: CLIControlRequest): Promise<void> {
        ...
        try {
            ...
            debugLogger.error(
                '[Session] SDK mode initialization failed:',
                error
            );
    ...
    private async initializeDirectMode(
        ...
        try {
            ...
            debugLogger.error(
                '[Session] Direct mode initialization failed:',
                error
            );
            this.failInitialization(
                error instanceof Error ? error : new Error(String(error))
    ...
    private handleControlRequestAsync(request: CLIControlRequest): void {
        ...
        void dispatcher.dispatch(request).catch((error) => {
            debugLogger.error(
                '[Session] Control request dispatch error:',
                error
            );
    ...
    private async processUserMessageQueue(): Promise<void> {
        ...
        while (
            ...
            try {
                ...
                debugLogger.error(
                    '[Session] Error processing user message:',
                    error
                );
    ...
    private async waitForAllPendingWork(): Promise<void> {
        ...
        if (this.dispatcher) {
            const pendingCount =
                this.dispatcher.getPendingIncomingRequestCount();
            if (pendingCount > 0) {
                debugLogger.debug(
        ...
        while (this.processingPromise) {
            ...
            try {
                ...
                debugLogger.error(
                    '[Session] Error in user message processing:',
                    error
                );
            }
        }
...
export async function runNonInteractiveStreamJson(
    ...
    let initialPrompt: CLIUserMessage | undefined;
