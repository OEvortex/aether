    ...
    describe('read', () => {
        ...
        it.each([
            ...
            },
            {
                ...
                input:
                    JSON.stringify({ type: 123, session_id: 'test-session' }) +
                    '\n',
                expectedError: 'Missing required "type" field'
            }
        ])('should throw StreamJsonParseError for $name', async ({
            input: inputLine,
            expectedError
        }) => {
            const input = new PassThrough();
        ...
        it('should use process.stdin as default input', () => {
            ...
            expect(
                (reader as unknown as { input: typeof process.stdin }).input
            ).toBe(process.stdin);
        });
        ...
        it('should use provided input stream', () => {
            ...
            expect(
                (reader as unknown as { input: typeof customInput }).input
            ).toBe(customInput);
        });
    });
});
