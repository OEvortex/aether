...
import type { Config, ToolCallRequestInfo } from '@aetherai/aether-core';
import {
    createDebugLogger,
    executeToolCall,
    FatalInputError,
...
export async function runNonInteractive(
    ...
    return promptIdContext.run(prompt_id, async () => {
        ...
        try {
            ...
            let initialPartList: PartListUnion | null =
                extractPartsFromUserMessage(options.userMessage);
                    ...
                    switch (slashCommandResult.type) {
                        ...
                        case 'message': {
                            ...
                            await emitNonInteractiveFinalMessage({
                                ...
                                isError:
                                    slashCommandResult.messageType === 'error',
                                adapter,
                                config,
                                ...
                                try {
                                    while (cronQueue.length > 0) {
                                        ...
                                        while (true) {
                                            ...
                                            adapter.finalizeAssistantMessage();
                                            totalApiDurationMs +=
                                                Date.now() - cronApiStartTime;

                                            if (
                                                cronToolCallRequests.length > 0
                                            ) {
