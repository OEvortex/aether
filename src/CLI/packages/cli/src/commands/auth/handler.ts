...
import { buildModelProvidersConfigFromProviderRegistry } from '../../ui/auth/providerSelection.js';
...
async function handleCodePlanAuth(
    ...
    writeStdoutLine(
        t('Processing Alibaba Cloud Coding Plan authentication...')
    );
    ...
    try {
        ...
        const modelProvidersConfig =
            buildModelProvidersConfigFromProviderRegistry(providers as any);
...
export async function showAuthStatus(): Promise<void> {
    try {
        ...
        if (!selectedType) {
            ...
            writeStdoutLine(
                t('Run one of the following commands to get started:\n')
            );
            writeStdoutLine(
                t(
        ...
        if (selectedType === AuthType.AETHER_OAUTH) {
            ...
            if (hasApiKey) {
                ...
                writeStdoutLine(
                    t('  Run `aether auth coding-plan` to re-configure.\n')
                );
            }
        } else {
            writeStdoutLine(
