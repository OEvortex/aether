...
describe('runNonInteractive', () => {
        ...
        mockConfig = {
            ...
            storage: {
                getProjectTempDir: vi
                    .fn()
                    .mockReturnValue('/test/project/.gemini/tmp')
            },
    ...
    it('should handle FatalInputError with custom exit code in JSON format', async () => {
        (mockConfig.getOutputFormat as Mock).mockReturnValue(OutputFormat.JSON);
        setupMetricsMock();
        const fatalError = new FatalInputError(
            'Invalid command syntax provided'
        );
    ...
    it('should emit tool results as user messages in stream-json format', async () => {
        ...
        const toolResultBlock =
            toolResultUserMessages[0]?.message?.content?.find(
                (block: unknown) =>
                    typeof block === 'object' &&
                    block !== null &&
                    'type' in block &&
