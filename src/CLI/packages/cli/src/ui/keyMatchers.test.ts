...
describe('keyMatchers', () => {
    ...
    describe('Custom key bindings', () => {
        it('should work with custom configuration', () => {
            ...
            expect(
                customMatchers[Command.HOME](createKey('h', { ctrl: true }))
            ).toBe(true);
            expect(customMatchers[Command.HOME](createKey('0'))).toBe(true);
            expect(
                customMatchers[Command.HOME](createKey('a', { ctrl: true }))
            ).toBe(false);
        });
        ...
        it('should support multiple key bindings for same command', () => {
            ...
            expect(matchers[Command.QUIT](createKey('q', { meta: true }))).toBe(
                true
            );
        });
    });
