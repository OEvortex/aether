    ...
    useEffect(() => {
        ...
        const parseKittyPrefix = (
            ...
            if (m) {
                ...
                const ctrl =
                    (modifierBits & MODIFIER_CTRL_BIT) === MODIFIER_CTRL_BIT;
                ...
                if (!ctrl) {
                    const keypadChar =
                        KITTY_KEYPAD_PRINTABLE_KEYCODE_TO_CHAR[keyCode];
                    if (keypadChar) {
                        return {
                            key: {
                                name: keypadChar,
                ...
                const keypadName =
                    KITTY_KEYPAD_FUNCTIONAL_KEYCODE_TO_NAME[keyCode];
                if (keypadName) {
                    return {
                        key: {
                            name: keypadName,
        ...
        const getCompleteCsiSequenceLength = (
            buffer: string
        ): number | null => {
            ...
            if (kittyProtocolEnabled) {
                if (
                    ...
                    while (kittySequenceBufferRef.current) {
                        ...
                        const nextStart =
                            kittySequenceBufferRef.current.indexOf(
                                `${ESC}[`,
                                1
                            );
                        if (nextStart > 0) {
        ...
        const flushRawBuffer = () => {
            ...
            while (cursor < data.length) {
                ...
                if (markerType === 'prefix') {
                    handleKeypress(
                        undefined,
                        createPasteKeyEvent('paste-start')
                    );
        ...
        const handleRawKeypress = (_data: Buffer) => {
            const data = Buffer.isBuffer(_data)
                ? _data
