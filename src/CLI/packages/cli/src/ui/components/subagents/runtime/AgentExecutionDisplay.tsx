...
export const AgentExecutionDisplay: React.FC<AgentExecutionDisplayProps> = ({
    ...
    const [displayMode, setDisplayMode] =
        React.useState<DisplayMode>('compact');
            ...
            <Box flexDirection="column">
                ...
                {data.status === 'completed' && data.executionSummary && (
                    <Box flexDirection="row" marginTop={1}>
                        <Text color={theme.text.secondary}>
                            Execution Summary:{' '}
                            {data.executionSummary.totalToolCalls} tool uses ·{' '}
                            {data.executionSummary.totalTokens.toLocaleString()}{' '}
                            tokens ·{' '}
...
const TaskPromptSection: React.FC<{
    ...
    const displayLines = showFull
        ? lines
        : lines.slice(0, MAX_TASK_PROMPT_LINES);
        ...
        <Box flexDirection="column">
            <Box flexDirection="row" marginBottom={1}>
                ...
                {shouldTruncate && displayMode === 'default' && (
                    <Text color={theme.text.secondary}>
                        {' '}
                        Showing the last {MAX_TOOL_CALLS} of {calls.length}{' '}
                        tools.
                    </Text>
                )}
            </Box>
