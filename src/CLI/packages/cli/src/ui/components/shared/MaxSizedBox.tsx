...
export const MaxSizedBox: React.FC<MaxSizedBoxProps> = ({
    ...
    const id = useId();
    const { addOverflowingId, removeOverflowingId } =
        useOverflowActions() || {};
    ...
    function visitRows(element: React.ReactNode) {
        ...
        debugReportError(
            'MaxSizedBox children must be <Box> elements',
            element
        );
    }
        ...
        <Box flexDirection="column" width={maxWidth} flexShrink={0}>
            {totalHiddenLines > 0 && overflowDirection === 'top' && (
                <Text color={theme.text.secondary} wrap="truncate">
                    ... first {totalHiddenLines} line
                    {totalHiddenLines === 1 ? '' : 's'} hidden ...
                </Text>
            )}
            {visibleLines}
            {totalHiddenLines > 0 && overflowDirection === 'bottom' && (
                <Text color={theme.text.secondary} wrap="truncate">
                    ... last {totalHiddenLines} line
                    {totalHiddenLines === 1 ? '' : 's'} hidden ...
                </Text>
            )}
        </Box>
