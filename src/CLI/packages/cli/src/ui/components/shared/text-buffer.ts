...
import { createDebugLogger, unescapePath } from '@aetherai/aether-core';
...
export function offsetToLogicalPos(
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const lineLengthWithNewline =
            lineLength + (i < lines.length - 1 ? 1 : 0);
        ...
        ([, startColInLogical], index) => {
            const nextStartColInLogical =
                index + 1 < segmentsForLogicalLine.length
                    ...
                    : Infinity;
            return (
                logicalCol >= startColInLogical &&
                logicalCol < nextStartColInLogical
            );
        }
    );
    ...
    switch (action.type) {
        ...
        case 'move': {
            ...
            if (
                ...
                const visualCursor = calculateVisualCursorFromLayout(
                    visualLayout,
                    [cursorRow, cursorCol]
                );
                ...
                const currentVisLineLen = cpLen(
                    visualLines[newVisualRow] ?? ''
                );
                ...
                if (visualToLogicalMap[newVisualRow]) {
                    const [logRow, logStartCol] =
                        visualToLogicalMap[newVisualRow];
                    return {
                        ...state,
                        cursorRow: logRow,
                        cursorCol: clamp(
        ...
        case 'delete_word_right': {
            ...
            if (cursorCol >= lineLen) {
                ...
                const nextWordStart = findNextWordStartInLine(
                    lineContent,
                    cursorCol
                );
...
export function useTextBuffer({
    ...
    const visualCursor = useMemo(
        () =>
            calculateVisualCursorFromLayout(visualLayout, [
                cursorRow,
                cursorCol
            ]),
