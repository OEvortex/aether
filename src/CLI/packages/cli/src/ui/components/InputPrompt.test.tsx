...
describe('InputPrompt', () => {
    ...
    describe('clipboard image paste', () => {
        ...
        beforeEach(() => {
            vi.mocked(clipboardUtils.clipboardHasImage).mockResolvedValue(
                false
            );
            ...
            vi.mocked(
                clipboardUtils.cleanupOldClipboardImages
    ...
    it('should complete a partial parent command', async () => {
        ...
        mockedUseCommandCompletion.mockReturnValue({
            ...
            showSuggestions: true,
            suggestions: [
                { label: 'memory', value: 'memory', description: '...' }
            ],
    ...
    describe('cursor-based completion trigger', () => {
        ...
        it('should handle multiple escaped spaces in paths', async () => {
            ...
            mockedUseCommandCompletion.mockReturnValue({
                ...
                suggestions: [
                    {
                        label: 'my long file name.md',
                        value: 'my long file name.md'
                    }
                ]
            });
    ...
    describe('multiline paste', () => {
        ...
        ])('should handle multiline paste $description', async ({
            pastedText
        }) => {
            const { stdin, unmount } = renderWithProviders(
                <InputPrompt {...props} />
    ...
    describe('command search (Ctrl+R when not in shell)', () => {
        it('passes newest-first user history to command search', async () => {
            ...
            expect(commandSearchCall?.[1]).toEqual([
                'newest',
                'middle',
                'oldest'
            ]);
