...
export const InputPrompt: React.FC<InputPromptProps> = ({
    ...
    const nextLargePastePlaceholder = useCallback(
        (charCount: number): string => {
            const activeIds =
                activePlaceholderIds.current.get(charCount) || new Set();
    ...
    const handleSubmitAndClear = useCallback(
        (submittedValue: string) => {
            ...
            if (pendingPastes.size > 0) {
                ...
                const escapedPlaceholders = placeholders.map(
                    (placeholderValue) =>
                ...
                finalValue = finalValue.replace(
                    placeholderRegex,
                    (matchedPlaceholder) =>
                        pendingPastes.get(matchedPlaceholder) ??
                        matchedPlaceholder
                );
            ...
            if (attachments.length > 0) {
                const attachmentRefs = attachments
                    .map(
                        (att) =>
                            `@${path.relative(config.getTargetDir(), att.path)}`
                    )
                    .join(' ');
    ...
    const handleInput = useCallback(
        (key: Key): boolean => {
            ...
            if (uiState.isFeedbackDialogOpen) {
                ...
                if (
                    (FEEDBACK_DIALOG_KEYS as readonly string[]).includes(
                        key.name
                    )
                ) {
                    return true;
