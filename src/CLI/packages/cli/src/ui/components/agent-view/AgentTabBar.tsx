...
import { AgentEventType, AgentStatus } from '@aetherai/aether-core';
    ...
    return (
        <Box flexDirection="row" paddingX={1}>
            ...
            {[...agents.entries()].map(([agentId, agent]) => {
                ...
                const { symbol, color: indicatorColor } =
                    statusIndicator(agent);
                ...
                return (
                    <Box key={agentId} marginLeft={1}>
                        <Text
                            ...
                            dimColor={!isFocused}
                            backgroundColor={
                                isActive ? theme.border.default : undefined
                            }
                            color={
                                isActive
                                    ? undefined
                                    : agent.color || theme.text.secondary
                            }
                        >
                            {` ${agent.modelId} `}
                        </Text>
