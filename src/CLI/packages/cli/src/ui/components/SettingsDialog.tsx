...
import { type Config, createDebugLogger } from '@aetherai/aether-core';
        ...
        (key) => {
            ...
            if (mode === 'settings') {
                ...
                if (editingKey) {
                    ...
                    if (name === 'right') {
                        setEditCursorPos((pos) =>
                            Math.min(cpLen(editBuffer), pos + 1)
                        );
                        return;
                    }
                    // Home and End keys
                ...
                if (name === 'up' || name === 'k') {
                    ...
                    const newIndex =
                        activeSettingIndex < items.length - 1
                            ? activeSettingIndex + 1
                            : 0;
                    setActiveSettingIndex(newIndex);
                    // Adjust scroll offset for wrap-around
                    ...
                    if (currentSetting) {
                        ...
                        if (!requiresRestart(currentSetting.value)) {
                            const immediateSettings = new Set([
                                currentSetting.value
                            ]);
                            const toSaveValue =
