...
import type { ChatRecord, Config } from '@aetherai/aether-core';
        ...
        if (
            ...
            if (hasOriginalContent || hasNewContent) {
                ...
                if (display.diffStat) {
                    ...
                    stats.linesRemoved +=
                        display.diffStat.model_removed_lines ?? 0;
                } else {
...
function calculateTokenStats(records: ChatRecord[]): {
    ...
    if (lastValidRecord) {
        const percent =
            (lastValidRecord.totalTokenCount /
                lastValidRecord.contextWindowSize) *
            100;
        return {
            totalTokens,
    ...
    const lastAssistantRecord = [...records]
        .reverse()
        .find(
            (r) => r.type === 'assistant' && r.contextWindowSize !== undefined
        );
...
async function extractMetadata(
    ...
    if (cwd) {
        const { getGitRepoName } = await import('@aetherai/aether-core');
                ...
                const usageMetadata:
                    ...
                    ? {
                          ...
                          cachedContentTokenCount:
                              usageMeta.usage.cachedReadTokens
                      }
                    : undefined;
