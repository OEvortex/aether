    ...
    describe('getStatusColor', () => {
        describe('with red threshold', () => {
            ...
            it('should return green for values >= green threshold', () => {
                expect(getStatusColor(90, thresholds)).toBe(
                    theme.status.success
                );
                expect(getStatusColor(80, thresholds)).toBe(
                    theme.status.success
                );
            });

            it('should return yellow for values < green and >= yellow threshold', () => {
                expect(getStatusColor(79, thresholds)).toBe(
                    theme.status.warning
                );
                expect(getStatusColor(50, thresholds)).toBe(
                    theme.status.warning
                );
            });
