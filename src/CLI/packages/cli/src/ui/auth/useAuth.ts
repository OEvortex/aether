        ...
        async (
            ...
            if (credentials?.providerId) {
                if (
                    ...
                    onAuthError(
                        t(
                            'API key required for the selected provider. Please choose the provider again and enter its API key.'
                        )
                    );
                    setIsAuthDialogOpen(true);
                    setIsAuthenticating(false);
        ...
        async (
            ...
            try {
                ...
                const modelIds = modelIdsInput
                    ...
                    .filter(
                        (id, index, array) =>
                            id.length > 0 && array.indexOf(id) === index
                    );
                if (!trimmedApiKey) {
                ...
                const newConfigs: ProviderModelConfig[] = modelIds.map(
                    (modelId) => ({
                        id: modelId,
