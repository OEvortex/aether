...
export const contextCommand: SlashCommand = {
    ...
    action: async (context: CommandContext, args?: string) => {
        ...
        const memoryFilesTokens = memoryFiles.reduce(
            (sum, f) => sum + f.tokens,
            0
        );
        ...
        const skillTool = allTools.find(
            (tool) => tool.name === ToolNames.SKILL
        );
        ...
        if (isEstimated) {
            ...
            displaySystemPrompt = Math.round(
                systemPromptTokens * overheadScale
            );
            ...
            const scaledMcpTotal = Math.round(
                mcpToolsTotalTokens * overheadScale
            );
            ...
            const scaleDetail = <T extends { tokens: number }>(
                items: T[]
            ): T[] =>
                overheadScale < 1
                    ? items.map((item) => ({
