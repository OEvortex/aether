    ...
    try {
        ...
        if (context.services.settings?.setValue) {
            try {
                ...
                debugLogger.warn(
                    'Failed to save output language setting:',
                    error
                );
            }
        }
        ...
        return {
            ...
            content: [
                t('Current UI language: {{lang}}', {
                    lang: formatUiLanguageDisplay(
                        currentUiLang as SupportedLanguage
                    )
                }),
                t('Current LLM output language: {{lang}}', {
                    lang: outputLangDisplay
                }),
                '',
