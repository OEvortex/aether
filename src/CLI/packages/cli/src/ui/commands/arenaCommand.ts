...
function buildArenaExecutionInput(
    ...
    const models: ArenaModelConfig[] = parsed.models.map((parsedModel) => {
        ...
        const registryModels =
            modelsConfig.getAvailableModelsForAuthType(authType);
        const resolved = registryModels.find(
            (m) => m.id === parsedModel.modelId
        );
        return {
    ...
    subCommands: [
        ...
        },
        {
            ...
            > => {
                ...
                const hasSuccessful = agents.some((a) =>
                ...
                if (trimmedArgs) {
                    ...
                    const result = await manager.applyAgentResult(
