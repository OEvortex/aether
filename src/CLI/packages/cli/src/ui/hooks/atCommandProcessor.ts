    ...
    while (currentIndex < query.length) {
        ...
        if (atIndex === -1) {
            ...
            if (currentIndex < query.length) {
                parts.push({
                    type: 'text',
                    content: query.substring(currentIndex)
                });
            }
            break;
        }
        ...
        while (pathEndIndex < query.length) {
            ...
            if (inEscape) {
                ...
                const nextChar =
                    pathEndIndex + 1 < query.length
                        ? query[pathEndIndex + 1]
                        : '';
...
export async function handleAtCommand({
    ...
    try {
        ...
        if (parts.length > 0 && !result.error) {
            ...
            onDebugMessage(
                'readManyFiles returned no content or empty content.'
            );
        }
