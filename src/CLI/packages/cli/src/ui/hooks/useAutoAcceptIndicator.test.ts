...
import { ApprovalMode, Config } from '@aetherai/aether-core';
...
describe('useAutoAcceptIndicator', () => {
    ...
    beforeEach(() => {
        ...
        (
            ...
            const instance: MockConfigInstanceShape = {
                ...
                >,
                isTrustedFolder: vi.fn().mockReturnValue(true) as Mock<
                    () => boolean
                >,
                getCoreTools: vi.fn().mockReturnValue([]) as Mock<
                    () => string[]
                >,
                getToolDiscoveryCommand: vi
                    .fn()
                    .mockReturnValue(undefined) as Mock<
                    () => string | undefined
                >,
                getTargetDir: vi.fn().mockReturnValue('.') as Mock<
                    () => string
                >,
                getApiKey: vi.fn().mockReturnValue('test-api-key') as Mock<
                    () => string
                >,
                getModel: vi.fn().mockReturnValue('test-model') as Mock<
                    () => string
                >,
                getSandbox: vi.fn().mockReturnValue(false) as Mock<
                    () => boolean | string
                >,
                getDebugMode: vi.fn().mockReturnValue(false) as Mock<
                    () => boolean
                >,
                getQuestion: vi.fn().mockReturnValue(undefined) as Mock<
                    () => string | undefined
                >,
                getFullContext: vi.fn().mockReturnValue(false) as Mock<
                    () => boolean
                >,
                getUserAgent: vi
                    .fn()
                    .mockReturnValue('test-user-agent') as Mock<() => string>,
                getUserMemory: vi.fn().mockReturnValue('') as Mock<
                    () => string
                >,
            ...
            instanceSetApprovalModeMock.mockImplementation(
                (value: ApprovalMode) => {
    ...
    describe('in untrusted folders', () => {
        ...
        it('should cycle from YOLO to PLAN when Shift+Tab is pressed', () => {
            ...
            expect(mockConfigInstance.getApprovalMode()).toBe(
                ApprovalMode.PLAN
            );
