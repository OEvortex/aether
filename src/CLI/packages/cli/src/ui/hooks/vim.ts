...
const vimReducer = (state: VimState, action: VimAction): VimState => {
    switch (action.type) {
        ...
        case 'INCREMENT_COUNT':
            return {
                ...state,
                count: state.count * DIGIT_MULTIPLIER + action.digit
            };
    ...
    const handleInput = useCallback(
        (key: Key): boolean => {
            ...
            try {
                ...
                debugLogger.warn(
                    'Malformed key input in vim mode:',
                    key,
                    error
                );
                return false;
