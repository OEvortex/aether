...
function matchesCommand(cmd: SlashCommand, query: string): boolean {
    return (
        ...
        cmd.altNames?.some(
            (alt) => alt.toLowerCase() === query.toLowerCase()
        ) ||
        false
    );
}
    ...
    useEffect(() => {
        ...
        if (commandsToSearch.length > 0) {
            const performFuzzySearch = async () => {
                ...
                if (!signal.aborted) {
                    const finalSuggestions = potentialSuggestions.map(
                        (cmd) => ({
    ...
    return useMemo(() => {
        ...
        if (hasTrailingSpace || exactMatchAsParent) {
            ...
            if (parserResult.isArgumentCompletion) {
                ...
                return {
                    start: query.length - partial.length,
                    end: query.length
                };
            }
        } else {
...
export function useSlashCompletion(props: UseSlashCompletionProps): {
    ...
    const { start: calculatedStart, end: calculatedEnd } =
        useCompletionPositions(query, parserResult);
