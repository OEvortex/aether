...
export const useGeminiStream = (
    ...
    const retryCountdownTimerRef = useRef<ReturnType<
        typeof setInterval
    > | null>(null);
        ...
        if (
            ...
            const turnCount =
                lastUserMessageIndex === -1
                    ? 0
                    : history.length - lastUserMessageIndex;
    ...
    const prepareQueryForGemini = useCallback(
        async (
            ...
            if (typeof query === 'string') {
                ...
                onDebugMessage(
                    `Received user query (${trimmedQuery.length} chars)`
                );
    ...
    const handleContentEvent = useCallback(
        (
            ...
            if (splitPoint === newGeminiMessageBuffer.length) {
                ...
                const beforeText = newGeminiMessageBuffer.substring(
                    0,
                    splitPoint
                );
        ...
        (userMessageTimestamp: number) => {
            ...
            if (pendingHistoryItemRef.current) {
                if (pendingHistoryItemRef.current.type === 'tool_group') {
                    const updatedTools =
                        pendingHistoryItemRef.current.tools.map((tool) =>
    ...
    const handleFinishedEvent = useCallback(
        (event: ServerGeminiFinishedEvent, userMessageTimestamp: number) => {
            ...
            const finishReasonMessages: Record<
                FinishReason,
                string | undefined
            > = {
    ...
    const handleCompletedTools = useCallback(
        async (completedToolCallsFromScheduler: TrackedToolCall[]) => {
                    ...
                    (
                        ...
                        if (isTerminalState) {
                            const completedOrCancelledCall = tc as
                                ...
                                | TrackedCancelledToolCall;
                            return (
                                completedOrCancelledCall.response
            ...
            const newSuccessfulMemorySaves =
