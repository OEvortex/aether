...
describe('useGeminiStream', () => {
    ...
    it('should submit tool responses when all tool calls are completed and ready', async () => {
        const toolCall1ResponseParts: Part[] = [
            { text: 'tool 1 final response' }
        ];
        const toolCall2ResponseParts: Part[] = [
    ...
    describe('Thought Reset', () => {
        ...
        it('should accumulate streamed thought descriptions', async () => {
            ...
            await waitFor(() => {
                expect(result.current.thought?.description).toBe(
                    'thinking more'
                );
            });
        });
        ...
        it('should show a retry countdown and update pending history over time', async () => {
            ...
            try {
                ...
                expect((errorItem as { hint?: string })?.hint).toContain(
                    'attempt 1/3'
                );
                ...
                expect(
                    (errorAfterOneSecond as { hint?: string })?.hint
                ).toContain('2s');
    ...
    describe('Loop Detection Confirmation', () => {
        ...
        it('should set loopDetectionConfirmationRequest when LoopDetected event is received', async () => {
            ...
            await waitFor(() => {
                ...
                expect(
                    typeof result.current.loopDetectionConfirmationRequest
                        ?.onComplete
                ).toBe('function');
            });
        });
    ...
    describe('HookSystemMessage Event', () => {
        it('should handle HookSystemMessage event and add stop_hook_system_message history item', async () => {
            ...
            await act(async () => {
                await result.current.submitQuery(
                    'test query with hook system message'
                );
            });
            ...
            await waitFor(() => {
                expect(mockAddItem).toHaveBeenCalledWith(
                    expect.objectContaining({
                        ...
                        message:
                            '🔄 Ralph iteration 3 | No completion promise set'
                    }),
                    expect.any(Number)
                );
            });
