...
import { getErrorMessage, getMCPServerPrompts } from '@aetherai/aether-core';
                ...
                const newPromptCommand: SlashCommand = {
                    ...
                    completion: async (
                        ...
                        const indexOfFirstSpace =
                            invocation.raw.indexOf(' ') + 1;
                        let promptInputs =
                            indexOfFirstSpace === 0
                                ? {}
                                : this.parseArgs(
                                      invocation.raw.substring(
                                          indexOfFirstSpace
                                      ),
                                      prompt.arguments
                                  );
                        ...
                        if (exactlyMatchingArgumentAtTheEnd.length === 1) {
                            if (
                                exactlyMatchingArgumentAtTheEnd[0] ===
                                partialArg
                            ) {
                                return [`${partialArg}"`];
                            }
    ...
    parseArgs(
        ...
        while ((match = namedArgRegex.exec(userArgs)) !== null) {
            ...
            if (match.index > lastIndex) {
                positionalParts.push(
                    userArgs.substring(lastIndex, match.index)
                );
            }
        ...
        while (
            (match = positionalArgRegex.exec(positionalArgsString)) !== null
        ) {
            // Extract the quoted or unquoted argument and remove escape chars.
