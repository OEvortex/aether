...
export class DataProcessor {
        ...
        const INSIGHT_SCHEMA = {
            ...
            properties: {
                ...
                brief_summary: {
                    type: 'string',
                    description:
                        'One sentence: what user wanted and whether they got it'
                }
            },
            required: [
                'underlying_goal',
    ...
    private aggregateFacetsData(facets: SessionFacets[]): {
        ...
        facets.forEach((facet) => {
            ...
            // Aggregate outcomes
            if (facet.outcome) {
                outcomesAgg[facet.outcome] =
                    (outcomesAgg[facet.outcome] || 0) + 1;
            }
    ...
    private prepareCommonPromptData(
        ...
        const frictionDetails = facets
            .filter(
                (f) => f.friction_detail && f.friction_detail.trim().length > 0
            )
    ...
    private async generateMetrics(
        ...
        for (let i = 0; i < totalFiles; i += BATCH_SIZE) {
            ...
            for (const fileInfo of batch) {
                try {
                    const records = await readJsonlFile<ChatRecord>(
                        fileInfo.path
                    );
        ...
        const analysisPromises = eligibleSessions.map(({ fileInfo, records }) =>
            limit(async () => {
                try {
                    ...
                    if (onProgress) {
                        const percent =
                            20 + Math.round((completed / total) * 60);
                        onProgress(
                            'Analyzing sessions',
                            percent,
                            `${completed}/${total}`
                    ...
                    logger.error(
                        `Error analyzing session file ${fileInfo.path}:`,
                        e
                    );
                    completed++;
                    if (onProgress) {
