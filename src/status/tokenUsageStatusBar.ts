...
export class TokenUsageStatusBar {
    ...
    private static _onDidChangeActiveModel =
        new vscode.EventEmitter<TokenUsageData>();
    static readonly onDidChangeActiveModel =
        TokenUsageStatusBar._onDidChangeActiveModel.event;
    ...
    private generateTooltip(data: TokenUsageData): vscode.MarkdownString {
        ...
        if (data.inputTokens === 0 && data.maxInputTokens === 0) {
            md.appendMarkdown(
                '💡 Displayed after sending any request provided by Copilot Helper\n'
            );
            return md;
        }
        ...
        md.appendMarkdown(
            `| **Usage** | **${data.percentage.toFixed(1)}%** ${usageString} |\n`
        );
        ...
        md.appendMarkdown('\n---\n');
        md.appendMarkdown(
            '💡 This data shows the context usage of the most recent request\n'
        );
