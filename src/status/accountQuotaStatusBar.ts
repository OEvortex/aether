...
} from '../accounts/accountQuotaCache';
...
export class AccountQuotaStatusBar extends BaseStatusBarItem<AccountQuotaStatusData> {
    ...
    protected generateTooltip(
        ...
        for (const account of data.accounts) {
            ...
            md.appendMarkdown(
                `${icon} **${account.accountName || account.accountId}** (${account.provider})\n`
            );
    ...
    protected async performApiQuery(): Promise<{
        success: boolean;
        data?: AccountQuotaStatusData;
        error?: string;
    }> {
        try {
            ...
            const exceededAccounts = states.filter(
                (s) => s.quotaExceeded
            ).length;
            const warningAccounts = states.filter(
                (s) => s.backoffLevel && s.backoffLevel > 0 && !s.quotaExceeded
            ).length;
            const activeAccounts = states.filter(
                (s) => !s.quotaExceeded
            ).length;
            ...
            Logger.error(
                '[AccountQuotaStatusBar] Failed to get quota data:',
                error
            );
    ...
    protected override async onInitialized(): Promise<void> {
        ...
        cache.onQuotaStateChange((event) => {
            StatusLogger.debug(
                `[${this.config.logPrefix}] Quota state changed for ${event.accountId}`
            );
