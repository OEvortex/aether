...
export abstract class BaseStatusBarItem<T> {
    ...
    private validateConfig(): void {
        ...
        for (const field of requiredFields) {
            if (!this.config[field]) {
                throw new Error(
                    `Invalid status bar configuration: ${field} cannot be empty`
                );
            }
        }
        ...
        if (typeof this.config.priority !== 'number') {
            throw new Error(
                'Invalid status bar configuration: priority must be a number'
            );
        }
    }
    ...
    protected abstract performApiQuery(): Promise<{
        success: boolean;
        data?: T;
        error?: string;
    }>;
    ...
    async initialize(context: vscode.ExtensionContext): Promise<void> {
        if (this.initialized) {
            StatusLogger.warn(
                `[${this.config.logPrefix}] Status bar item already initialized`
            );
            return;
        }
        ...
        this.shouldShowStatusBar()
            ...
            .catch((error) => {
                StatusLogger.error(
                    `[${this.config.logPrefix}] Failed to check display conditions`,
                    error
                );
            });
        ...
        StatusLogger.info(
            `[${this.config.logPrefix}] Status bar item initialization completed`
        );
    }
    ...
    delayedUpdate(delayMs = 2000): void {
        ...
        StatusLogger.debug(
            `[${this.config.logPrefix}] Setting delayed update, will execute in ${finalDelayMs / 1000} seconds`
        );

        this.updateDebouncer = setTimeout(async () => {
            try {
                StatusLogger.debug(
                    `[${this.config.logPrefix}] Executing delayed update`
                );
                this.lastDelayedUpdateTime = Date.now();
                await this.performInitialUpdate();
            } catch (error) {
                StatusLogger.error(
                    `[${this.config.logPrefix}] Delayed update failed`,
                    error
                );
            } finally {
    ...
    dispose(): void {
        ...
        StatusLogger.info(
            `[${this.config.logPrefix}] Status bar item destroyed`
        );
    }
    ...
    private async performRefresh(): Promise<void> {
        try {
            if (this.statusBarItem && this.lastStatusData) {
                const previousText = this.getDisplayText(
            ...
            StatusLogger.error(
                `[${this.config.logPrefix}] Refresh failed`,
                error
            );
    ...
    protected async executeApiQuery(isManualRefresh = false): Promise<void> {
        if (this.isLoading) {
            StatusLogger.debug(
                `[${this.config.logPrefix}] Query in progress, skipping duplicate call`
            );
            return;
        }
        ...
        if (!isManualRefresh && this.lastStatusData) {
            try {
                ...
                StatusLogger.debug(
                    `[${this.config.logPrefix}] Cache data format incompatible, continuing with refresh`
                );
            }
        }
        ...
        try {
            StatusLogger.debug(
                `[${this.config.logPrefix}] Starting usage query...`
            );
            ...
            if (result.success && result.data) {
                if (this.statusBarItem) {
                    ...
                    StatusLogger.info(
                        `[${this.config.logPrefix}] Usage query successful`
                    );
                }
            } else {
                ...
                StatusLogger.warn(
                    `[${this.config.logPrefix}] Usage query failed: ${errorMsg}`
                );
            }
        } catch (error) {
            StatusLogger.error(
                `[${this.config.logPrefix}] Status bar update failed`,
                error
            );
    ...
    private startCacheUpdateTimer(): void {
        this.cacheUpdateTimer = setInterval(() => {
            if (this.shouldRefresh()) {
                StatusLogger.debug(
                    `[${this.config.logPrefix}] Cache timer triggered refresh`
                );
                this.executeApiQuery(false).catch((error) => {
                    StatusLogger.error(
                        `[${this.config.logPrefix}] Cache timer refresh failed`,
                        error
                    );
                });
            }
    ...
    private registerLeaderPeriodicTask(): void {
        LeaderElectionService.registerPeriodicTask(async () => {
            if (this.shouldRefresh()) {
                StatusLogger.debug(
                    `[${this.config.logPrefix}] Leader periodic task triggered refresh`
                );
                await this.executeApiQuery(false);
            }
        });
