...
export class Fetcher implements IFetcher {
    ...
    async fetch(url: string, options: FetchOptions): Promise<Response> {
        ...
        if (
            options?.method !== 'POST' ||
            url.endsWith('/completions') === false
        ) {
        ...
        try {
            ...
            if (provider !== 'chatjimmy') {
                ...
                if (!apiKey) {
                    logger.error(
                        `[Fetcher] ${provider} API key not configured`
                    );
            ...
            const getText = async (): Promise<string> => {
                ...
                if (bodyConsumed) {
                    throw new Error(
                        'Response body has already been consumed as stream'
                    );
                }
                bodyConsumed = true;
            ...
            const getJson = async (): Promise<unknown> => {
                ...
                try {
                    ...
                    logger.error(
                        '[Fetcher.ResponseWrapper] JSON parsing failed:',
                        e
                    );
                    throw e;
                }
            ...
            const getBody = async (): Promise<Readable | null> => {
                if (isChatJimmyFim) {
                    if (bodyConsumed) {
                        if (cachedText !== null) {
                            const cleanedText =
                                this.stripChatJimmyStats(cachedText);
