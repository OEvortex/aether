    ...
    for (const model of json.models) {
        ...
        if (isKimi(hay)) {
            ...
            const changedK =
                beforeIn !== model.maxInputTokens ||
                beforeOut !== model.maxOutputTokens ||
                (model.capabilities && model.capabilities.imageInput !== k25);
            if (changedK) {
                changed = true;
                changedModels.push({
                    id: model.id,
...
if (RESULTS.length === 0) {
    console.log(
        'No changes necessary — all Kimi and MiniMax models already match the rule.'
    );
} else {
    for (const r of RESULTS) {
        ...
        for (const m of r.changedModels) {
            console.log(
                `  ${m.id}: in ${m.beforeIn} -> ${m.afterIn}, out ${m.beforeOut} -> ${m.afterOut}` +
