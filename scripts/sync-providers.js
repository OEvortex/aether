...
const KNOWN_PROVIDERS_FILE = path.join(ROOT, 'src', 'utils', 'knownProviders.ts');
const PROVIDER_KEYS_FILE = path.join(ROOT, 'src', 'types', 'providerKeys.ts');
const ACCOUNT_MANAGER_FILE = path.join(ROOT, 'src', 'accounts', 'accountManager.ts');
const PACKAGE_JSON_FILE = path.join(ROOT, 'package.json');

const OAUTH_PROVIDERS = new Set(['antigravity', 'codex', 'geminicli', 'qwencli']);
const OAUTH_ONLY_PROVIDERS = new Set(['antigravity', 'geminicli', 'qwencli']);
...
const NO_BASE_URL_CHAT_PROVIDER_CONFIG = new Set([
    'antigravity',
    'codex',
    'geminicli',
    'qwencli'
...
function parseKnownProviders() {
    ...
    for (const node of sourceFile.statements) {
        ...
        for (const declaration of node.declarationList.declarations) {
            if (!ts.isIdentifier(declaration.name) || declaration.name.text !== 'knownProviderOverrides') {
                continue;
            }
    ...
    if (!providersNode) {
        throw new Error('Could not find knownProviderOverrides in src/utils/knownProviders.ts');
    }
    ...
    for (const property of providersNode.properties) {
        ...
        const displayName = getStringProperty(providerConfig, 'displayName') || providerId;
        ...
        const hasOpenAI = Boolean(openaiNode && ts.isObjectLiteralExpression(openaiNode));
        const hasAnthropic = Boolean(anthropicNode && ts.isObjectLiteralExpression(anthropicNode));
...
function syncAccountManagerFile(accountProviderItems) {
    ...
    const pattern = /private static providerConfigs = new Map<string, ProviderAccountConfig>\(\[[\s\S]*?\t\]\);/;
    if (!pattern.test(source)) {
        throw new Error('Could not find providerConfigs map block in src/accounts/accountManager.ts');
    }
...
function ensureBaseUrlProperty(providerProperties) {
    ...
    providerProperties.baseUrl = {
        type: 'string',
        description: 'Custom base URL or proxy endpoint (optional). Overrides the default API endpoint.',
        default: ''
    };
}
...
function syncPackageJson(knownProviders) {
    ...
    const providerById = new Map(knownProviders.map((provider) => [provider.id, provider]));
    ...
    packageJson.activationEvents = [...staticActivationEvents, ...providerActivationEvents];
    ...
    const existingCommandSet = new Set(retainedCommands.map((command) => command.command));
    ...
    for (const providerId of providerIds) {
        ...
        syncedProperties[`chp.${providerId}.baseUrl`] = createBaseUrlProperty(provider);
    }
    ...
    for (const providerId of providerIds) {
        ...
        syncedProperties[`chp.${providerId}.sdkMode`] = createSdkModeProperty(provider);
    }

    packageJson.contributes.configuration.properties = syncedProperties;

    const existingLanguageModelProviders = Array.isArray(packageJson.contributes?.languageModelChatProviders)
    ...
    const syncedLanguageModelProviders = providerIds.map((providerId) => {
        const provider = providerById.get(providerId);
        if (!provider) {
            return null;
        }
    ...
    packageJson.contributes.languageModelChatProviders = syncedLanguageModelProviders;
...
function run() {
    ...
    console.log(`Synced ${knownProviders.length} providers from knownProviders.ts`);
